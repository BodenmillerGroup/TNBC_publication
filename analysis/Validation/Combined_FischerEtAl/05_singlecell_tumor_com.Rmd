...
#based on cluster categories (n=627677)
...
cluster_channel = c("CK5","CK8_18","CD15","KRT14","cMYC","HER2","ER","PR","p53","AR","GATA3","EGFR","p_mTOR","CK7","Vimentin","Ki67","HLADR", "CAIX", "S6", "c_Cas3_PARP", "pH3", "H3K27me3")

#cell_state_channel = c()

other_immune_channel = c("Bcl2", "CD44","panCK","Ecad","SMA","Fibronectin","CD68","CD3","CD20","CD11c","CD45","CD20","CD8a","CD4","CD31_vWF")
...
# Run batch-correction for PID 

```{r}
# set.seed(22)
# out <- fastMNN(sce, batch = sce$PID,
#                auto.merge = FALSE,
...
system.time(
harmony_emb <- HarmonyMatrix(mat, sce$PID, do_pca = TRUE)
) 

reducedDim(sce, "harmony_PID") <- harmony_emb
```
...
sce_sub = runTSNE(sce_sub[rowData(sce_sub)$cluster_channel,], dimred = "harmony_PID", name = "TSNE_harmony", external_neighbors=TRUE, BPPARAM = MulticoreParam())


## Visualize TSNE before and after batch correction
TSNE_beforebatch <- plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "PID")+
  scale_color_manual(values = metadata(sce)$colors$PID)+
  guides(color = "none")
TSNE_afterbatch <- plotReducedDim(sce_sub, dimred = "TSNE_harmony", colour_by = "PID")+
  scale_color_manual(values = metadata(sce)$colors$PID)+
  guides(color = "none")

TSNE_beforebatch+TSNE_afterbatch
```
...
#mat <- reducedDim(sce, "harmony_PID")
mat <- t(assay(sce, "exprs")[rowData(sce)$cluster_channel,])
...
saveRDS(sce, "/mnt/tnbc_volume/TNBC_all/data_analysis/05_sce_TNBC_tumor_com_notbatch.rds")
...
sce <- readRDS("/mnt/tnbc_volume/TNBC_all/data_analysis/05_sce_TNBC_tumor_com_notbatch.rds")
...
sce <- readRDS("/mnt/tnbc_volume/TNBC_all/data_analysis/05_sce_TNBC_tumor.rds")
...
sce <- readRDS("/mnt/tnbc_volume/TNBC_all/data_analysis/05_sce_TNBC_tumor_com.rds")
...
anno <- anno %>% mutate(metacluster = ifelse(pre_metacluster == "1" & cluster_110 %in% c("13","6","27","29"), "8", 
                                       ifelse(pre_metacluster == "1" & cluster_110 %in% c("4","12","15","21","8"), "9", 
                                              ifelse(pre_metacluster == "1" & cluster_110 %in% c("34","24","11","10"), "1", pre_metacluster))))
anno$metacluster <- factor(anno$metacluster)

#add (pre) metacluster to sce
sce$metacluster <- anno$pre_metacluster[match(sce$cluster_110,anno$cluster_110)]
...
saveRDS(sce, "/mnt/tnbc_volume/TNBC_all/data_analysis/05_sce_TNBC_tumor.rds")
```
...
UMAP_clus <- plotReducedDim(sce_sub, dimred = "UMAP", colour_by = "cluster_10¨10")+
  scale_color_manual(values = metadata(sce)$colors$cluster_10¨10)
...
#sce$metacluster <- sce$cluster_130
...
fit <- survfit(Surv(OS_months, status_OS) ~ MC9, data = Cox_df_mean)
print(fit)
...
fit <- survfit(Surv(OS_months, status_OS) ~ MC7, data = Cox_df_mean)
print(fit)
...
```{r OS survival_curves}
#Plot survival curves
fit <- survfit(Surv(DFS_months, status_DFS) ~ MC5, data = Cox_df_mean)
print(fit)
...
fit <- survfit(Surv(DFS_months, status_DFS) ~ MC7, data = Cox_df_mean)
print(fit)
...
plotPixels(cur_images,
           ...
           save_plot = list(filename = paste0("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/Tumor_clusters_viz/Tumor_viz_metacluster_",i,".png"), scale = 2)
           )
...
plotPixels(cur_images,
           ...
           legend = list(colour_by.title.cex = 0.7,
                         colour_by.labels.cex = 0.7),
           save_plot = list(filename = paste0("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/TME_clusters_viz/TME_viz_immune_metacluster_",i,".png"), scale = 2)
           )
...
pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_ClusterTesting.pdf",width = 10,height = 5)
p1+p2+p3
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_Silhouette_Cluster100.pdf",width = 12,height = 10)
p4
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/05_tumor/TNBC_Tumor_Heatmap_Cluster_Metacluster.pdf",width = 14,height = 9)
h
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_HeatmapCluster_Scales.pdf",width = 20,height = 8)
h_exprs+h_min_max
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_ClusterAnno.pdf",width = 5,height = 8)
h_cluster_anno
dev.off()
...
pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_ExpressionCor.pdf",width = 10,height = 8)
corrplot(cor(cor_sce,method = "spearman"), method="color", col=brewer.pal(n=8, name="RdBu"),  
         type="upper", order="hclust",hclust.method = "ward.D2", 
         addCoef.col = "black", addCoefasPercent = F,# Add coefficient of correlation
         tl.col="black", tl.srt=40, #Text label color and rotation
...
pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_TSNECluster_Metacluster.pdf",width = 12,height = 6)
TSNE_clus+TSNE_metaclus
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_TSNEExpression.pdf",width = 12,height = 6)
exp_plots_TSNE
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_UMAPExpression.pdf",width = 12,height = 6)
exp_plots_UMAP
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_MetaClusterPID.pdf",width = 10,height = 10)
p_clusterPID/p_metaclusterPID
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_PIDMetaCluster_ClusterCor.pdf",width = 12,height = 10)
(p_PID_cluster+SID_cluster_cor_plot)/
(p_PID_metacluster + SID_metacluster_cor_plot)
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/05_tumor/TNBC_Tumor_Meta_HR_OS.pdf",width = 8,height = 7)
hr_prop/
hr_mean
dev.off()


pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/05_tumor/TNBC_Tumor_MC4_KaplanMeier_OS.pdf",width = 6,height = 6, onefile = FALSE)
p1
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_ClusterMeta_HR_DFS.pdf",width = 10,height = 8)
hr_prop/
hr_cat/
hr_cat_GMM
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_Heatmap_metaClus_PIDgroups.pdf",width = 8,height = 8)
h_g_meta
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_Heatmap_Clus_PIDgroups.pdf",width = 8,height = 8)
h_g_clus
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_HeatmapPIDgroups_mean.pdf",width = 11,height = 7)
h_g_rich
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_HeatmapPIDgroupsCLR.pdf",width = 11,height = 7)
h_g_1
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_HeatmapPIDgroupsCLR_mean.pdf",width = 11,height = 7)
h_g_rich_clr
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_PIDgroups_Clus_OS.pdf",width = 8,height = 5)
plot_OS
dev.off()

pdf("/mnt/tnbc_volume/TNBC_all/data_analysis/01_figures/06_tumor/TNBC_249_Tumor_CC_PIDgroups_Clus_DFS.pdf",width = 8,height = 5)
plot_DFS
dev.off()
