...
Code related to following cleaned **Zenodo** objects: MDACC_ES.rds, MDACC_EXP.rds, GeneSignatures.rds	

# Validation

Data analysis scripts for data analysis in the validation cohorts. 

# Script : GSE25066 - Microarray data analysis for chemotherapy response in breast cancer cohort

Here, we are analyzing microarray gene expression data for the MDACC chemotherapy cohort. 
...
expset$TNBC_assign_lehmann <- lehmann_GSE$TNBC_FILTER[match(expset$PID, lehmann_GSE$PID)] #Use this as TNBC assignment
...
GMM_df <- data.frame(class_epi = ifelse(exprs_summarized["KRT5",] > 10.4, "KRT5+", "KRT5-"),
...
GMM_df <- data.frame(class_epi = ifelse(exprs_summarized["KRT7",] > 11.8, "KRT7+", "KRT7-"),
...
chrom_sig_up_rel <- chrom_sig_up[chrom_sig_up %in% rownames(exprs_summarized)]
# Filter for availachrome genes
df <- t(exprs_summarized[chrom_sig_up_rel ,]) %>% as.data.frame
# Z-score gene expression 
df <- as.data.frame(scale(df))
...
GMM_df <- data.frame(class_epi = ifelse(exprs_summarized["HLA-DRA",] > 13, "HLA-DRA+", "HLA-DRA-"),
...
```{r}
## 2. BL signature per PID group 
INFg_sig_up_rel <- INFg_sig_up[INFg_sig_up %in% rownames(exprs_summarized)]
# Filter for available genes
df <- t(exprs_summarized[INFg_sig_up_rel ,]) %>% as.data.frame
# Z-score gene expression 
df <- as.data.frame(scale(df))
...
```{r}
## 2. BL signature per PID group 
CTA_sig_up_rel <- CTA_sig_up[CTA_sig_up %in% rownames(exprs_summarized)]
# Filter for available genes
df <- t(exprs_summarized[CTA_sig_up_rel ,]) %>% as.data.frame
# Z-score gene expression 
df <- as.data.frame(scale(df))
...
```{r}
## 2. BL signature per PID group 
INFab_sig_up_rel <- INFab_sig_up[INFab_sig_up %in% rownames(exprs_summarized)]
# Filter for available genes
df <- t(exprs_summarized[INFab_sig_up_rel ,]) %>% as.data.frame
# Z-score gene expression 
df <- as.data.frame(scale(df))
