...
Code related to following cleaned **Zenodo** objects: sce_TUM.rds 

# Discovery 

Data analysis scripts for IMC data analysis in the discovery cohort. 

# Script 5: Tumor analysis

Here, we will perform comprehensive single cell tumor cell analysis.
...
fraction_anno <- HeatmapAnnotation(
  ...
  MC_fractions = anno_barplot(cur_df, gp=gpar(fill=metadata(sce)$colors$metacluster[colnames(cur_df)]),
                          ...
                          pretreatment = anno_simple(as.character(anno$pretreatment), col=metadata(sce)$colors$pretreatment, border = TRUE), diversity = anno_simple(anno$shannon_div, col = metadata(sce)$colors$diversity_col, border = TRUE),
                          border = TRUE, 
                          #annotation_name_rot = 90,
                          gap = unit(1,"mm"),
                          which = "row")
...
write.csv(plot_df_mean, "/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/supp_tables/Cox_Uni_tumorMC.csv")
...
shannon_plot <- data.frame(shannon_div = diversity(shannon_df),
                           shannon_equi = (diversity(shannon_df)/log(length(unique(sce$metacluster)))),
...
#pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/05_tumor/TNBC_Tumor_Diversity_Pretreatment.pdf", width = 3.5, height = 5)
