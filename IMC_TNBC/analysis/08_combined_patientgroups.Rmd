...
cur_surv <- colData(sce) %>% as.data.frame() %>% select(PID, PID_CN_groups, OS_months, DFS_months, status_DFS, status_OS) %>% filter(!PID_CN_groups %in% c("8")) %>%
  unique()
...
#Based on proportions
covariates <- Cox_df %>% select(-status_DFS,-DFS_months, -age, -grade, -pT_simple, -pN_simple, -pM_simple, -SCP8) %>% colnames

multi_formula <- as.formula(paste("Surv(DFS_months, status_DFS) ~ age + grade + pT_simple + pN_simple + pM_simple +", paste(covariates, collapse = " + "), "+ SCP8"))
...
#Shannon diversity index
shannon_mean
shannon_mean[rownames(mixing_pheno)]
