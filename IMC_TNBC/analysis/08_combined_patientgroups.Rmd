...
```{r}
spatial_CD8 <- colData(sce) %>% as.data.frame() %>% select(PID_metagroups,spatial_CD8_pheno) %>% group_by(PID_metagroups) %>% table() %>% as.data.frame
spatial_CD8 <- spatial_CD8 %>% group_by(PID_metagroups) %>% mutate(fra = Freq/sum(Freq))

ggplot(spatial_CD8, aes(x = PID_metagroups, y = fra, fill = spatial_CD8_pheno))+
...
```{r}
CK5_7 <- colData(sce) %>% as.data.frame() %>% select(PID_metagroups,CK5_7_score) %>% group_by(PID_metagroups) %>% table() %>% as.data.frame
CK5_7 <- CK5_7 %>% group_by(PID_metagroups) %>% mutate(fra = Freq/sum(Freq))

ggplot(CK5_7, aes(x = PID_metagroups, y = fra, fill = CK5_7_score))+
  geom_bar(stat = "identity", color = "black")+
  theme_classic()+
  ylab("Fraction of PID")
