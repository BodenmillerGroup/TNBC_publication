...
cur_surv <- colData(sce) %>% as.data.frame() %>% select(PID, PID_metagroups, OS_months, DFS_months, status_DFS, status_OS) %>% filter(PID_metagroups != "1") %>% unique()
...
cur_surv <- colData(sce) %>% as.data.frame() %>% select(PID, PID_metagroups_density, OS_months, DFS_months, status_DFS, status_OS) %>% filter(!PID_metagroups_density %in% c("1","2","12")) %>% unique()
...
#Retrieve PID groups
row_groups <- foreach(i = 1:9, .combine = rbind) %do%{
data.frame(PID = rownames(PID_metagroups[row_order(h_g)[[i]],]), PID_group = i)
}
...
PID_metagroups_col <- setNames(colorRampPalette(brewer.pal(9, "Accent"))(length(anno$PID_metagroups)), anno$PID_metagroups)                   
...
## Create color scheme for mixing and CD8 pheno 
mixing_pheno_col <- setNames(c("steelblue1","purple","navy"), colnames(mixing_pheno))                   
spatial_CD8_pheno_col <- setNames(c("steelblue1","navy","firebrick"), colnames(spatial_CD8_pheno))                   
