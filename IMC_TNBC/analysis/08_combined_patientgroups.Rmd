...
sce$BL_CD8_group_merg[sce$BL_CD8_group_merg %in% c("L_cold/excluded","BL_cold/excluded")] <- "L_BL"
sce$BL_CD8_group_merg[sce$BL_CD8_group_merg %in% c("B_cold/excluded","N_cold/excluded")] <- "B_N"
```
...
hb <- PID_spatial_tumor %>% ungroup %>% select(metacluster_combined,BL_CD8_group_merg,fra) %>% group_by(BL_CD8_group_merg, metacluster_combined) %>% summarize(mean = mean(fra)) %>% pivot_wider(names_from = metacluster_combined, values_from = mean) %>% column_to_rownames("BL_CD8_group_merg")

hb_scaled <- scale(as.matrix(hb))

Heatmap(hb, col = viridis(100))
Heatmap(hb_scaled)
