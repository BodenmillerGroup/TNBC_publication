...
set.seed(22)
cur_sam <- sample(unique(sce$sample_id),100)
...
unique(sce$spatial_context)
summary(is.na(sce$spatial_context))
...
edges <- buildEdgeList(sce, combined = TRUE)

## 1. Create edgelist (code for above function)
data <- colData(sce) %>% as.data.frame() %>% select(spatial_context,sample_id) %>% group_by(spatial_context) %>% table() %>% as.data.frame

data_wide <- data %>% pivot_wider(values_from = Freq, names_from = spatial_context) %>% column_to_rownames("sample_id")

registerDoMC(cores=detectCores()-2) #perform foreach loop on multiple cores with %dopar%

combined <- FALSE #decide in which way to generate edgelist
...
anno <- data.frame(spatial_context = unique(SC_SID$spatial_context),
           length = list_length,
           sum = SC_SID %>% group_by(spatial_context) %>% summarise(sum = sum(Freq)) %>% pull(sum),
           n_samples = SC_SID %>% group_by(spatial_context) %>% filter(Freq != 0) %>% count() %>% pull(n)
           )

#spatial context abundance filter criteria
...
anno <- data.frame(spatial_context = unique(data$spatial_context), 
                   length = str_count(unique(data$spatial_context), "[0-9]"),
                   sum = data %>% group_by(spatial_context) %>% summarise(sum = sum(Freq)) %>% pull(sum), 
                   n_samples = data %>% group_by(spatial_context) %>% filter(Freq != 0) %>% count() %>% pull(n)
)

anno <- unique(c(edges$from,edges$to))
