...
cluster_mean_sce <- aggregateAcrossCells(sce, ids = sce$KRT5_7_score,
                                 ...
                                 subset.row = rowData(sce)$Symbol %in%  c("KRT5","KRT7","KRT15","KRT8","KRT18","KRT19", "KRT14","KRT17"))
                                #rowData(sce)$Symbol %in%  c("KRT5","KRT7","KRT15","KRT8","KRT18","KRT19", "KRT14") & rowData(sce)$Symbol %in% breast_majorCT$Gene)

# Heatmap body colors 
col_exprs <- colorRamp2(c(0,1,2,3), c("#440154FF", "#31688EFF", "#35B779FF", "#FDE725FF"))


#rowData(cluster_mean_sce) <- 

CK_type <- data.frame(CK = c("KRT5","KRT14","KRT17","KRT7","KRT8","KRT18","KRT19","KRT15"), 
           type = c(rep("basal_like",3), rep("luminal_like",4),"stem_like"))
  
rowData(cluster_mean_sce)$type <- CK_type$type[match(rowData(cluster_mean_sce)$Symbol, CK_type$CK)]  
...
column_order <- column_order(Heatmap(assay(cluster_mean_sce, "logcounts"),
        col = viridis(100),
        name = "mean logcounts",
        row_title = c("basal_like","luminal_like","stem_like"), 
        show_column_names =  TRUE, 
        clustering_method_rows = "ward.D2",
        clustering_method_columns = "ward.D2",
        row_split = rowData(cluster_mean_sce)$type,
