...
#based on cluster categories (n=339287)
sce <- sce[,colData(sce)$analysis_cat == "non_tumor"]
sce
```

## Perform overclustering and hierarchical clustering
Next, we will run Rphenograph (Jaccard-based weights, Louvain clustering as community detection algorithm) clustering with different resolution parameters (k=30,50,50,50). 
...
clusters_3 <- igraph::cluster_louvain(snn.sce)
sce$cluster_125 <- as.factor(clusters_3$membership)
...
pairwiseRand(sce$cluster_50,sce$cluster_50, mode = "index")
pairwiseRand(sce$cluster_50,sce$cluster_50, mode = "index")
...
Observation:  
- Highest Silhouette width for k=50 => selected
...
spatial <- colData(sce) %>% as.data.frame %>% select(area,eccentricity,cluster_50,n_neighbors)
spatial <- spatial %>% select(-cluster_50) %>% aggregate(by = list(cluster_50 = spatial$cluster_50), FUN = mean) %>% column_to_rownames("cluster_50")
#define min_max fct
...
anno <- colData(cluster_mean_sce) %>% as.data.frame %>% select(cluster_50, ncells)

# ##Set Preliminary cluster names (after visual inspection)
# cluster_names <- c("CK5/7+","EGFR/Ki67/p53+","CK5/KRT14+","CK5+/Ki67low","Vim+","Vim_low","Low","Vim/CK7+","Ki67+","Low","Vim/Ki67+","KRT14+/Vim","CD15+","Ki67/Vim+/CK5_low","CK7+","CK5/KRT14/Vim+",
#                    "Vim/p53/cMYC+","Ki67/CK7/8/18+","CK8/18low","Her2low","CK7/8/18+/mTORlow","CK5/8/18/CD15/Vim+","ER/GATA3+","GATA3+")
# sce$cluster_names <- anno$names[match(sce$cluster_50,anno$cluster_50)]

# #Annotate clusters
# anno_clus <- as.character(sce$cluster_50)
# 
# anno_clus[anno_clus == "1"] <- "CK5/CK7+"
# anno_clus[anno_clus == "2"] <- "EGFR/Ki67/p53+"
# anno_clus[anno_clus == "3"] <- "CK14/CK5/Vim+"
# anno_clus[anno_clus == "4"] <- "Vim/HLADR+"
# anno_clus[anno_clus == "5"] <- "Vim/CK7low"
# anno_clus[anno_clus == "6"] <- "CK5low"
# anno_clus[anno_clus == "7"] <- "HLADRlow"
# anno_clus[anno_clus == "8"] <- "Ki67/p53+"
# anno_clus[anno_clus == "9"] <- "Ki67+"
# anno_clus[anno_clus == "10"] <- "CK14/Vim+"
# anno_clus[anno_clus == "11"] <- "CK7low"
# anno_clus[anno_clus == "12"] <- "CK14/Ki67+"
# anno_clus[anno_clus == "13"] <- "Ki67+/CK5/7low"
# anno_clus[anno_clus == "14"] <- "CD15+"
# anno_clus[anno_clus == "15"] <- "Vim/p53/cMYC+"
# anno_clus[anno_clus == "16"] <- "GATA3low"
# anno_clus[anno_clus == "17"] <- "CK7/8/18+/Ki67low"
# anno_clus[anno_clus == "18"] <- "CD15/AR+"
# anno_clus[anno_clus == "19"] <- "CK7/HLADRlow"
# anno_clus[anno_clus == "20"] <- "HLADR+"
# anno_clus[anno_clus == "21"] <- "CK7/8/18+"
# anno_clus[anno_clus == "22"] <- "HER2+"
# anno_clus[anno_clus == "23"] <- "Vim/CK5/8/18/CD15+"
# anno_clus[anno_clus == "24"] <- "ER/GATA3+"
# anno_clus[anno_clus == "25"] <- "CK14+"
# anno_clus[anno_clus == "26"] <- "HLADR/CK5/CK14+"
# 
...
grade <- colData(sce) %>% as.data.frame() %>% select(cluster_50,grade) %>% group_by(cluster_50) %>% table() %>% as.data.frame
grade <- grade %>% group_by(cluster_50) %>% mutate(fra = Freq/sum(Freq)) 
grade <- grade %>% select(-Freq) %>% pivot_wider(id_cols = cluster_50,names_from = grade,values_from = fra) %>% column_to_rownames("cluster_50")

# # Proportion of lehmann subtype per cluster (as bubble plot)
# subtype <- colData(sce) %>% as.data.frame() %>% select(cluster_50,lehmann_subtype) %>% group_by(cluster_50) %>% table() %>% as.data.frame
# subtype <- subtype %>% group_by(cluster_50) %>% mutate(fra = Freq/sum(Freq)) 
# subtype <- subtype %>% select(-Freq) %>% pivot_wider(id_cols = cluster_50,names_from = lehmann_subtype,values_from = fra) %>% column_to_rownames("cluster_50")
# 
...
cluster_SID <- colData(sce) %>% as.data.frame() %>% select(cluster_50,sample_id) %>% group_by(cluster_50) %>% table() %>% as.data.frame
n_SID <- cluster_SID %>% filter(Freq>0) %>% group_by(cluster_50) %>% count(name = "n_SID") %>% column_to_rownames("cluster_50")

cluster_PID <- colData(sce) %>% as.data.frame() %>% select(cluster_50,PID) %>% group_by(cluster_50) %>% table() %>% as.data.frame
n_PID <- cluster_PID %>% filter(Freq>0) %>% group_by(cluster_50) %>% count(name = "n_PID") %>% column_to_rownames("cluster_50")

# Cluster categories contributing to clusters
cluster_CC <- colData(sce) %>% as.data.frame() %>% select(cluster_50,cluster_category) %>% group_by(cluster_50) %>% table() %>% as.data.frame
cluster_CC <- cluster_CC %>% group_by(cluster_50) %>% mutate(fra = Freq/sum(Freq)) 
cluster_CC <- cluster_CC %>% select(-Freq) %>% pivot_wider(id_cols = cluster_50,names_from = cluster_category,values_from = fra) %>% column_to_rownames("cluster_50")
...
col <- list(cluster_50 = colorRampPalette(brewer.pal(9, "Paired"))(length(unique(anno$cluster_50))))
col_clusters <- col[[1]]
names(col_clusters) = row_order_clusters
col_clusters <- list(col_clusters)
names(col_clusters) <- "cluster_50"
...
h <-Heatmap(t(assay(cluster_mean_sce, "min_max")),
  ...
  Heatmap(spatial_norm,
        ...
        clustering_method_rows = "ward.D2",
        clustering_method_columns = "ward.D2")+
  rowAnnotation(cluster_50 = anno_simple(unfactor(anno$cluster_50), pch = unfactor(anno$cluster_50),border=TRUE,col=col_clusters$cluster_50))+
...
h_cluster_anno <- rowAnnotation(cluster_50 = anno_simple((row_order_clusters), pch = as.character(row_order_clusters),border=TRUE,col=col_clusters$cluster_50))+
...
#panCK and GMM_CK
UMAP_clus <- plotReducedDim(sce_sub, dimred = "UMAP", colour_by = "cluster_50")+
  scale_color_manual(values = col_clusters$cluster_50)

TSNE_clus <- plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "cluster_50")+
  scale_color_manual(values = col_clusters$cluster_50)
...
cluster_SID <- colData(sce) %>% as.data.frame() %>% select(cluster_50,sample_id) %>% group_by(cluster_50) %>% table() %>% as.data.frame
cluster_SID <- cluster_SID %>% group_by(cluster_50) %>% mutate(fra = Freq/sum(Freq))
cluster_PID <- colData(sce) %>% as.data.frame() %>% select(cluster_50,PID) %>% group_by(cluster_50) %>% table() %>% as.data.frame
cluster_PID <- cluster_PID %>% group_by(cluster_50) %>% mutate(fra = Freq/sum(Freq))
...
p_clusterPID <- ggplot(cluster_PID) +
  geom_tile(color = "black",aes(cluster_50, fra, fill = PID), position = "stack")+
...
p_clusterSID <- ggplot(cluster_SID) +
  geom_tile(color = "black",aes(cluster_50, fra, fill = sample_id), position = "stack")+
...
SID_cluster <- colData(sce) %>% as.data.frame() %>% select(cluster_50,sample_id) %>% group_by(sample_id) %>% table() %>% as.data.frame
SID_cluster <- SID_cluster %>% group_by(sample_id) %>% mutate(fra = Freq/sum(Freq))
PID_cluster <- colData(sce) %>% as.data.frame() %>% select(cluster_50,PID) %>% group_by(PID) %>% table() %>% as.data.frame
PID_cluster <- PID_cluster %>% group_by(PID) %>% mutate(fra = Freq/sum(Freq))

p_PID_cluster <- ggplot(PID_cluster) +
  geom_tile(color = "black",aes(PID, fra, fill = cluster_50), position = "stack")+
  scale_fill_manual(values=col_clusters$cluster_50) + 
...
p_SID_cluster <- ggplot(SID_cluster) +
  geom_tile(color = "black",aes(sample_id, fra, fill = cluster_50), position = "stack")+
    scale_fill_manual(values=col_clusters$cluster_50) + 
  theme_classic(base_size = 15)+
  ylab("Fraction")+
  xlab("Sample_ID")+
...
SID <- SID_cluster_cor %>% filter(PID %in% i) %>% select(-Freq,-PID) %>% pivot_wider(id_cols = cluster_50,names_from = sample_id,values_from = fra) %>% column_to_rownames("cluster_50")
...
PID_groups <- PID_cluster %>% 
  filter(!cluster_50 %in% c("26","23")) %>% 
  pivot_wider(id_cols = "PID",names_from = "cluster_50",values_from = "fra") %>% column_to_rownames("PID")
...
col_main = viridis(50)
col_cluster_groups <- col_clusters$cluster_50[names(col_clusters$cluster_50) %in% col_anno$cluster_50]
...
h_g <- Heatmap(PID_groups, name = "Cluster fraction", 
        ...
        row_title_rot = 0) %v%
  columnAnnotation(cluster_50 = anno_simple(unfactor(col_anno$cluster_50), pch = unfactor(col_anno$cluster_50),
...
row_groups <- foreach(i = 1:17, .combine = rbind) %do%{
...
# PID_groups_clr_1 <- PID_cluster_1 %>% pivot_wider(id_cols = "PID",names_from = "cluster_50",values_from = "fra") %>% column_to_rownames("PID")
...
col_anno <- anno %>% filter(!cluster_50 %in% c("26","23"))
...
#kmeans clustering
#set.seed(22)
#out_k <- kmeans(PID_clr,centers = 14,nstart = 5000,iter.max = 5000)
...
# # Proportion of grade per patient group
# grade <- row_anno_1 %>% select(PID_groups,grade) %>% group_by(PID_groups) %>% table() %>% as.data.frame
# grade <- grade %>% group_by(PID_groups) %>% mutate(fra = Freq/sum(Freq)) 
# grade <- grade %>% select(-Freq) %>% pivot_wider(id_cols = PID_groups,names_from = grade,values_from = fra) %>% column_to_rownames("PID_groups")
# 
# # Proportion of lehmann subtype per patient group
# subtype <- row_anno_1  %>% select(PID_groups,lehmann_subtype) %>% group_by(PID_groups) %>% table() %>% as.data.frame
# subtype <- subtype %>% group_by(PID_groups) %>% mutate(fra = Freq/sum(Freq)) 
# subtype <- subtype %>% select(-Freq) %>% pivot_wider(id_cols = PID_groups,names_from = lehmann_subtype,values_from = fra) %>% column_to_rownames("PID_groups")

# #Color scheme
# col <- list(PID_groups = colorRampPalette(brewer.pal(9, "Set3"))(length(unique(row_anno_1$PID_groups))),
#            lehmann_subtype = colorRampPalette(brewer.pal(9, "Set2"))(length(unique(row_anno_1$lehmann_subtype))),
#            grade = colorRampPalette(brewer.pal(2, "Reds"))(length(unique(row_anno_1$grade)))
# )
# 
# col_list_1 <- foreach(i=1:3) %do% {
#   color= col[[i]]
#   names(color)= sort(unique(row_anno_1[,names(col)[i]]))
#   color
# }
# names(col_list_1) <- c(names(col))
# 
# ha_right  <-  rowAnnotation(PID_groups = as.factor(row_anno_1$PID_groups), col = col_list_1[1], border = TRUE)
...
#Retrieve PID groups (for agnes)
row_groups_1 <- foreach(i = 1:17, .combine = rbind) %do%{
...
PID <- PID_groups_cor %>% filter(PID_group == i) %>% select(-Freq,-PID_group,-PID_group_clr) %>% pivot_wider(id_cols = cluster_50,names_from = PID,values_from = fra) %>% column_to_rownames("cluster_50")
...
PID <- PID_groups_cor %>% filter(PID_group_clr == i) %>% select(-Freq,-PID_group,-PID_group_clr) %>% pivot_wider(id_cols = cluster_50,names_from = PID,values_from = fra) %>% column_to_rownames("cluster_50")
cor <- cor(PID,method = "spearman")
cor <- unique(as.vector(cor))
cor <- cor[!cor == 1]
data.frame(cor = cor, PID_group = as.character(i), category = "PID_group")
...
shannon_df <- PID_groups_cor %>% pivot_wider(id_cols = PID,names_from = cluster_50, values_from = fra) %>% column_to_rownames("PID")
...
pdf("/Volumes/rcc_volume/TNBC/data_analysis/01_figures/TNBC_249_TME_CC_HeatmapCluster.pdf",width = 13,height = 8)
h
dev.off()
