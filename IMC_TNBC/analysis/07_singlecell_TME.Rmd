...
#Cluster colors 
col_clus <- colorRampPalette(brewer.pal(9,"Paired"))(length(unique(sce$cluster_75)))

#By clusters for one method (here k=75)
#1. Silhouette width [-1,1]
#Clusters with large positive silhouette widths are well-separated from other clusters
sil.approx <- approxSilhouette(t(assay(sce, "exprs")), clusters=sce$cluster_75)
sil.data <- as.data.frame(sil.approx)
sil.data$closest <- factor(ifelse(sil.data$width > 0, sce$cluster_75, sil.data$other))
sil.data$cluster <- sce$cluster_75
...
Observation:  
- Highest Silhouette width for k=75 => selected
...
cluster_mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster_75,
                                 statistics = "mean",
                                 use.assay.type = "exprs",
                                 subset.row = rowData(sce)$cluster_channel)

cellstate_mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster_75,
                                 statistics = "mean",
                                 use.assay.type = "exprs",
                                 subset.row = rowData(sce)$cell_state_channel)

other_mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster_75,
                                 statistics = "mean",
                                 use.assay.type = "exprs",
...
```{r cluster merg}
sce$cluster_75_merg <- ifelse(sce$cluster_75 %in% c("8","15"), "8_15", sce$cluster_75)
sce$cluster_75_merg <- ifelse(sce$cluster_75_merg %in% c("11","17"), "11_17", sce$cluster_75_merg)
sce$cluster_75_merg <- ifelse(sce$cluster_75_merg %in% c("1","3"), "1_3", sce$cluster_75_merg)
sce$cluster_75_merg <- as.factor(sce$cluster_75_merg)
unique(sce$cluster_75_merg)
```
...
#Aggregrate across cells with the mean (median is also possible)
cluster_mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster_75_merg,
                                 statistics = "mean",
                                 use.assay.type = "min_max",
                                 subset.row = rowData(sce)$cluster_channel)

cellstate_mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster_75_merg,
                                 statistics = "mean",
                                 use.assay.type = "min_max",
                                 subset.row = rowData(sce)$cell_state_channel)

other_mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster_75_merg,
                                 statistics = "mean",
                                 use.assay.type = "min_max",
                                 subset.row = rowData(sce)$other_epi_channel)
...
spatial <- colData(sce) %>% as.data.frame %>% select(area,eccentricity,cluster_75_merg,n_neighbors)
spatial <- spatial %>% select(-cluster_75_merg) %>% aggregate(by = list(cluster_75_merg = spatial$cluster_75_merg), FUN = mean) %>% column_to_rownames("cluster_75_merg")
#define min_max fct
...
anno <- colData(cluster_mean_sce) %>% as.data.frame %>% select(cluster_75_merg, ncells)

#Annotate clusters (after visual inspection)
anno_clus <- as.character(sce$cluster_75_merg)

anno_clus[anno_clus == "1_3"] <- "SMA+"
anno_clus[anno_clus == "2"] <- "Unclear"
anno_clus[anno_clus == "4"] <- "Unclear_2"
anno_clus[anno_clus == "5"] <- "Fibro/Vim+"
anno_clus[anno_clus == "6"] <- "Vim+"
anno_clus[anno_clus == "7"] <- "Fibro/Ecad+"
anno_clus[anno_clus == "8_15"] <- "CD8_Tcells"
anno_clus[anno_clus == "9"] <- "CD4_Tcells"
anno_clus[anno_clus == "10"] <- "Granu"
anno_clus[anno_clus == "11_17"] <- "Bcells"
anno_clus[anno_clus == "12"] <- "Macrophages"
anno_clus[anno_clus == "13"] <- "Ecad+"
anno_clus[anno_clus == "14"] <- "Endo_1"
anno_clus[anno_clus == "16"] <- "Endo_2"

anno_clus <- data.frame(anno_clus = anno_clus, origin = as.character(sce$cluster_75_merg))

anno$names <- anno_clus$anno_clus[match(anno$cluster_75_merg,anno_clus$origin)]
sce$cluster_names <- anno_clus$anno_clus[match(sce$cluster_75_merg,anno_clus$origin)]


###4. Patient metadata - grade, number of SID/PID

# Proportion of grade per cluster
grade <- colData(sce) %>% as.data.frame() %>% select(cluster_75_merg,grade) %>% group_by(cluster_75_merg) %>% table() %>% as.data.frame
grade <- grade %>% group_by(cluster_75_merg) %>% mutate(fra = Freq/sum(Freq)) 
grade <- grade %>% select(-Freq) %>% pivot_wider(id_cols = cluster_75_merg,names_from = grade,values_from = fra) %>% column_to_rownames("cluster_75_merg")

# Number of contributing patients/samples per cluster
cluster_SID <- colData(sce) %>% as.data.frame() %>% select(cluster_75_merg,sample_id) %>% group_by(cluster_75_merg) %>% table() %>% as.data.frame
n_SID <- cluster_SID %>% filter(Freq>0) %>% group_by(cluster_75_merg) %>% count(name = "n_SID") %>% column_to_rownames("cluster_75_merg")

cluster_PID <- colData(sce) %>% as.data.frame() %>% select(cluster_75_merg,PID) %>% group_by(cluster_75_merg) %>% table() %>% as.data.frame
n_PID <- cluster_PID %>% filter(Freq>0) %>% group_by(cluster_75_merg) %>% count(name = "n_PID") %>% column_to_rownames("cluster_75_merg")

# Cluster categories contributing to clusters
cluster_CC <- colData(sce) %>% as.data.frame() %>% select(cluster_75_merg,cluster_category) %>% group_by(cluster_75_merg) %>% table() %>% as.data.frame
cluster_CC <- cluster_CC %>% group_by(cluster_75_merg) %>% mutate(fra = Freq/sum(Freq)) 
cluster_CC <- cluster_CC %>% select(-Freq) %>% pivot_wider(id_cols = cluster_75_merg,names_from = cluster_category,values_from = fra) %>% column_to_rownames("cluster_75_merg")
...
col <- list(cluster_75_merg = colorRampPalette(brewer.pal(12, "Accent"))(length(unique(anno$cluster_75_merg))))
col_clusters <- col[[1]]
names(col_clusters) = unique(anno$cluster_75_merg)[row_order_clusters]
col_clusters <- list(col_clusters)
names(col_clusters) <- "cluster_75_merg"
...
h <-Heatmap(t(assay(cluster_mean_sce, "min_max")),
  ...
  Heatmap(spatial_norm,
        ...
        clustering_method_rows = "ward.D2",
        clustering_method_columns = "ward.D2")+
  rowAnnotation(cluster_75_merg = anno_simple(unfactor(anno$cluster_75_merg), pch = unfactor(anno$cluster_75_merg),border=TRUE,col=col_clusters$cluster_75_merg))+
...
h_cluster_anno <- rowAnnotation(cluster_75_merg = anno_simple(unfactor((unique(anno$cluster_75_merg)[row_order_clusters])), pch = as.character(unfactor((unique(anno$cluster_75_merg)[row_order_clusters]))),border=TRUE,col=col_clusters$cluster_75_merg))+
...
#panCK and GMM_CK
UMAP_clus <- plotReducedDim(sce_sub, dimred = "UMAP", colour_by = "cluster_75_merg")+
  scale_color_manual(values = col_clusters$cluster_75_merg)

TSNE_clus <- plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "cluster_75_merg")+
  scale_color_manual(values = col_clusters$cluster_75_merg)
...
dittoHeatmap(sce_sub, 
             ...
             assay = "exprs", 
             scale = "none", 
             order.by = "cluster_75_merg", 
             heatmap.colors = viridis(50), 
             annot.by = c("cluster_75_merg"),
            annot.colors=col_clusters$cluster_75_merg[order((names(col_clusters$cluster_75_merg)))])
```
...
cluster_SID <- colData(sce) %>% as.data.frame() %>% select(cluster_75_merg,sample_id) %>% group_by(cluster_75_merg) %>% table() %>% as.data.frame
cluster_SID <- cluster_SID %>% group_by(cluster_75_merg) %>% mutate(fra = Freq/sum(Freq))
cluster_PID <- colData(sce) %>% as.data.frame() %>% select(cluster_75_merg,PID) %>% group_by(cluster_75_merg) %>% table() %>% as.data.frame
cluster_PID <- cluster_PID %>% group_by(cluster_75_merg) %>% mutate(fra = Freq/sum(Freq))
...
p_clusterPID <- ggplot(cluster_PID) +
  geom_tile(color = "black",aes(cluster_75_merg, fra, fill = PID), position = "stack")+
...
p_clusterSID <- ggplot(cluster_SID) +
  geom_tile(color = "black",aes(cluster_75_merg, fra, fill = sample_id), position = "stack")+
...
SID_cluster <- colData(sce) %>% as.data.frame() %>% select(cluster_75_merg,sample_id) %>% group_by(sample_id) %>% table() %>% as.data.frame
SID_cluster <- SID_cluster %>% group_by(sample_id) %>% mutate(fra = Freq/sum(Freq))
PID_cluster <- colData(sce) %>% as.data.frame() %>% select(cluster_75_merg,PID) %>% group_by(PID) %>% table() %>% as.data.frame
PID_cluster <- PID_cluster %>% group_by(PID) %>% mutate(fra = Freq/sum(Freq))

p_PID_cluster <- ggplot(PID_cluster) +
  geom_tile(color = "black",aes(PID, fra, fill = cluster_75_merg), position = "stack")+
  scale_fill_manual(values=col_clusters$cluster_75_merg) + 
...
p_SID_cluster <- ggplot(SID_cluster) +
  geom_tile(color = "black",aes(sample_id, fra, fill = cluster_75_merg), position = "stack")+
    scale_fill_manual(values=col_clusters$cluster_75_merg) + 
  theme_classic(base_size = 15)+
  ylab("Fraction")+
  xlab("Sample_ID")+
...
SID <- SID_cluster_cor %>% filter(PID %in% i) %>% select(-Freq,-PID) %>% pivot_wider(id_cols = cluster_75_merg,names_from = sample_id,values_from = fra) %>% column_to_rownames("cluster_75_merg")
...
cluster_SID %>% filter(cluster_75_merg == "13") %>% filter(!fra == 0) %>% arrange(desc(fra))
