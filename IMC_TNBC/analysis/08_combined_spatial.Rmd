...
sce <- readRDS("/mnt/rcc_volume/TNBC/data_analysis/sce_TNBC_249_08.rds")
int_metadata(sce) <- list(version = "1.16.0")

sce
#Subset sce
set.seed(1995)
cur_sam <- sample(unique(sce$sample_id),10)
...
library(data.table)
system.time(
sce <- detectSpatialContext_DT(sce, 
                            threshold = 0.9, 
                            entry = "aggregatedNeighbors",
                            name = "spatial_context")
)

## 4. Filter spatial contexts
sce <- filterSpatialContext(object = sce, 
                     entry = "spatial_context", 
                     sample_id = "sample_id",
                     n_cells_threshold = 100,
                     n_samples_threshold = 20,
                     name = "spatial_context_filtered")

## 5. Plot spatial context graphs
plotSpatialContext(object = sce, 
