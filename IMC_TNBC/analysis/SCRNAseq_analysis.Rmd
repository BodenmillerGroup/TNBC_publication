...
mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster_30,
                                 ...
                                 subset.row = rowData(sce)$Symbol %in% c("EPCAM","KRT5","KRT8","KRT7","KRT18"))
...
anno_cluscat <- anno_cluscat %>% 
  mutate(cluster_category = ifelse(max >= 0.4,"Epithelial","Non-Epithelial"))
...
Heatmap((t(assay(mean_sce, "logcounts"))))
