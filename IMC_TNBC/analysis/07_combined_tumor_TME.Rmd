...
# Select 8 clusters
set.seed(22)
tumor_cur_clusters <- kmeans(cur_dat, centers = 8, nstart = 1000)$cluster
...
# Select 8 clusters
set.seed(22)
stroma_cur_clusters <- kmeans(cur_dat, centers = 6, nstart = 1000)$cluster
...
unique(sce$milieu_tumor_6)
length(unique(sce$milieu_tumor_6))
...
#kmeans
set.seed(22)
cur_k$mixing_pheno <- kmeans(cur_k$mixing_score, 2, nstart = 100)$cluster

cur_k$mixing_pheno <- ifelse(cur_k$mixing_pheno == "1", "compartmentalized", "mixed")
...
## Create color scheme
mixing_pheno_col <- setNames(c("navy","purple","steelblue1"), unique(sce$mixing_pheno))
...
DA <- DA %>% mutate(cluster = rownames(DA), sign = DA$FDR<0.05, result = ifelse(DA$logFC > 0 & DA$FDR < 0.1, "UP",ifelse(DA$logFC < 0 & DA$FDR < 0.1, "DOWN", "NOTSIG"))) %>% 
  arrange(desc(logFC))
...
sce$mixing_pheno_merg <- colData(sce) %>% 
  as.data.frame %>% 
  select(mixing_pheno) %>% 
  mutate(mixing_pheno_merg = ifelse(mixing_pheno %in% c("cold","compartmentalized"),"cold_compartmentalized", mixing_pheno)) %>% pull(mixing_pheno_merg)
...
#relevant patient metadata (Relevel so that pT 1/2 is baseline)
meta <- colData(sce)[match(colnames(abundances), sce$PID),]
meta$mixing_pheno_merg <- relevel(x=factor(meta$mixing_pheno_merg), ref = "cold_compartmentalized")
...
ggplot(DA, aes(x = logFC, y = logCPM))+
  ...
  geom_label_repel(data = DA %>% filter(FDR<0.1), aes(label = cluster))+
  scale_color_manual(values = metadata(sce)$colors$metacluster_combined)+
  xlab("logFC (cold/compartmentalized vs mixed)")+
  ylab("Abundance (logCPM)")+
  theme_classic()+
  geom_vline(xintercept = 0, linetype = "dashed")+
  labs(color = "cluster")
