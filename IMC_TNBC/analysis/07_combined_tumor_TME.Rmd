...
### Test for differential interaction of CD8 T cells between conditions 
...
## More B cell patches in 
## Average number of patches per PID 
...
ggplot(plot_df, aes(x = spatial_CD8_pheno_merg, y = n_Bpatch))+
...
## 
colData(sce) %>% as.data.frame %>% select(PID, spatial_CD8_pheno_merg, patch_B_cell) %>% unique() %>% 
  
  
  group_by(spatial_CD8_pheno_merg) %>% 
  summarise(patch_inflamed = sum(metacluster_combined == "inflamed" & !is.na(patch_B_cell)),
            patch_cold = sum(metacluster_combined == "cold/excluded" & !is.na(patch_B_cell)),
            nopatch_inflamed = sum(metacluster_combined == "inflamed" & is.na(patch_B_cell)),
            nopatch_cold = sum(metacluster_combined == "cold/excluded" & is.na(patch_B_cell)))
...
#Abundance vs log fold change (best?)
diffabun_plot_cd8_1 <- ggplot(DA, aes(x = logFC, y = logCPM))+
  geom_point(aes(col= cluster, 
                 size = -log10(FDR)))+
  geom_label_repel(data = DA %>% filter(FDR<0.1), aes(label = cluster))+
  scale_color_manual(values = metadata(sce)$colors$metacluster_combined)+
