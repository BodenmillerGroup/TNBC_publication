...
size_tumor_plot <- size_plot %>% filter(str_detect(patch_name,"patch_tumor_")) %>% group_by(PID,patch_name) %>% mutate(mean_PID = mean(n), median_PID = median(n)) %>% select(patch_name,mean_PID) %>% unique()

ggplot(size_tumor_plot, aes(x = reorder(patch_name,mean_PID), y = log2(mean_PID)))+
  geom_boxplot(outlier.color = NA)+
...
ggplot(cur_dat, aes(x = CD8Tcell_type, y = Ki67))+
  geom_boxplot()+
  stat_compare_means(method = "kruskal.test")+
  theme_classic()+
  ylab("Ki67 expression")
  
# Aggregate expression per PID 
T_mean_PID <- aggregateAcrossCells(cur_sce, ids = cur_sce$PID,
                                 statistics = "mean",
                                 use.assay.type = "exprs",
                                 subset.row = rowData(cur_sce)$use_channel)

plotExpression(T_mean_PID, 
               features = c("Ki67"),
...
### Could use GMM to divide between Ki67+ and negative and then compare?
```
...
# 1. Difference of cell phenotype distances to B cell patches BETWEEN nodal status
ggplot(dist_df, aes(mean, color = metacluster_combined))+
  geom_density(size = 0.8)+
  geom_rug()+
  facet_wrap(~pN_merg)+
...
ggplot(dist_df, aes(x = reorder(pN_merg, mean), y = mean))+
  geom_boxplot()+
  geom_jitter(aes(color = pN_merg), size = 3, position=position_jitter(w=0.1,h=0.1))+
  facet_wrap(~metacluster_combined)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90), axis.title.x = element_blank())+
  ylab("Mean distance to MC8 cell patch (per image)")+
...
```{r}
#What kind of tumor cells dominate the tumors where basal-like luminal cells are present? 

#PID with at least 5 basoluminal cells 
cur_sample <- colData(sce) %>% as.data.frame %>% filter(metacluster_combined == "tumor_8") %>% group_by(PID) %>% count() %>% filter(n>5) %>% pull(PID)

cur_sce <- sce[,sce$PID %in% cur_sample]

cur_sce <- cur_sce[,cur_sce$analysis_cat == "tumor"]

dittoBarPlot(cur_sce, var = "metacluster_combined", group.by = "PID")+
  scale_fill_manual(values = metadata(sce)$colors$metacluster_combined, limits = force)+
  theme(axis.text.x = element_blank())

dittoBarPlot(cur_sce, var = "spatial_metacommunity", group.by = "PID")+
  #scale_fill_manual(values = metadata(sce)$colors$metacluster_combined, limits = force)+
  theme(axis.text.x = element_blank())
```
