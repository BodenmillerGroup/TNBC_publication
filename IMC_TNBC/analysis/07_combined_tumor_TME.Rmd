...
PID_patch <- colData(sce) %>% as.data.frame() %>% select(metacluster_combined,PID) %>% group_by(PID) %>% table() %>% as.data.frame
PID_patch <- PID_patch %>% group_by(PID) %>% filter(PID %in% patch_CK57_PID) %>% filter(str_detect(metacluster_combined,"tumor")) %>% mutate(fra = Freq/sum(Freq)) 
...
PID_patch_abundant <- PID_patch %>% mutate(max = max(fra)) %>% filter(fra == max) %>% ungroup() %>% count(metacluster_combined) %>% mutate(fra = n/sum(n), category = "cat")
