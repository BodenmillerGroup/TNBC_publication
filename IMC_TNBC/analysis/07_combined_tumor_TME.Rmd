...
# Visualize 
for_plot <- prop.table(table(tumor_sce$tumor_meta_community, tumor_sce$metacluster_combined), margin = 1)
rownames(for_plot) <- paste0("tumor_SMC_",rownames(for_plot))

pheatmap(for_plot, show_rownames = TRUE, scale = "column")
```
...
# Visualize 
for_plot <- prop.table(table(stroma_sce$stroma_meta_community, stroma_sce$metacluster_combined), margin = 1)
...
plotSpatial(sce[,sce$sample_id %in% cur_sam], img_id = "sample_id", node_color_by = "patch_tumor_6") #Patch
plotSpatial(sce[,sce$sample_id %in% cur_sam], img_id = "sample_id", node_color_by = "distToCells")+
  scale_color_gradientn(colors = pal)# Milieu

#Visualize tumor patches and CD8 T cells
cur_sce <- sce[,sce$sample_id == "ZTMA249.2_ZTMA249_ID_197_BX5Y10_171" & sce$metacluster_tumor_combined %in% c("tumor","CD8_T_cell")]

plotSpatial(cur_sce, img_id = "sample_id", node_color_by = "patch_tumor")
plotSpatial(cur_sce, img_id = "sample_id", node_color_by = "metacluster_tumor_combined")
plotSpatial(cur_sce, img_id = "sample_id", node_color_by = "distToCells")+
  scale_color_gradientn(colors = pal)


test_df <- colData(sce) %>% as.data.frame %>% filter(!is.na(distToCells) & metacluster_tumor_combined == "CD8_T_cell") %>% select(sample_id, metacluster_tumor_combined, distToCells) %>% group_by(sample_id, metacluster_tumor_combined) #%>% summarize(mean = mean(distToCells))
  
test_CD8 <- colData(sce) %>% as.data.frame %>% filter(metacluster_combined == "CD8_T_cell") %>% select(PID, metacluster_combined, distToCells) %>% group_by(PID, metacluster_combined) %>% count()

test_tumor <- colData(sce) %>% as.data.frame %>% filter(metacluster_tumor_combined == "tumor") %>% select(PID, metacluster_tumor_combined, distToCells) %>% group_by(PID, metacluster_tumor_combined) %>% count()

test_df$CD8_n <- test_CD8$n[match(test_df$PID, test_CD8$PID)]
test_df$tumor_n <- test_tumor$n[match(test_df$PID, test_tumor$PID)]

test_df$CD8_tumor_ratio <- (test_df$CD8_n/test_df$tumor_n)*100
test_df <- test_df %>% filter(metacluster_tumor_combined == "CD8_T_cell")

ggplot(test_df, aes(mean, CD8_tumor_ratio))+
  geom_point()

ggplot(test_df, aes(n, color = metacluster_combined))+
  geom_density(size = 0.8)+
  geom_rug()+
  xlab("Number of CD8_T_cells per image")+
  ggtitle("patch_tumor")+
  theme_classic()

#Boxplot
ggplot(test_df, aes(x = reorder(metacluster_combined, mean), y = mean))+
  geom_boxplot(aes(fill = metacluster_combined))+
...
#Density plot  
ggplot(test_df, aes(mean, color = metacluster_tumor_combined))+
  geom_density(size = 0.8)+
  geom_rug()+
...
CD8_df <- test_df %>% filter(metacluster_tumor_combined == "CD8_T_cell")
                   
ggplot(test_df %>% filter(metacluster_tumor_combined == "CD8_T_cell"), aes(mean, color = metacluster_tumor_combined))+
  geom_density(size = 0.8)+
  geom_rug()+
  #scale_color_manual(values = metadata(sce)$colors$metacluster_combined)+
  xlab("Mean distance to patch (per image)")+
...
```{r dist to patch}
sce <- minDistToCells(sce, img_id = "sample_id", x_cells = !is.na(sce$patch_tumor_6), BPPARAM = MulticoreParam(workers = 10))
...
test_df <- colData(sce) %>% as.data.frame %>% filter(!is.na(distToCells) & metacluster_combined != "tumor_6") %>% select(sample_id, metacluster_combined, distToCells) %>% group_by(sample_id, metacluster_combined) %>% summarize(mean = mean(distToCells))
...
```{r patch-milieu}
# Milieu detection ("expanded" patch)

library(doParallel)
registerDoParallel(cores = 3)

foreach(i = unfactor(unique(sce$metacluster_combined)))%dopar%{ 
  
  sce <- patchDetection(sce,
               patch_cells = sce$metacluster_combined == i,
               colPairName = "knn_interaction_graph",
               min_patch_size = 15,
               name = paste0("milieu_",i),
               expand_by = 30, 
               img_id = "sample_id",
...
milieu_image <- colData(sce) %>% as.data.frame %>% select(sample_id, milieu_tumor_6) %>% filter(!is.na(milieu_tumor_6)) %>% group_by(sample_id) %>% unique() %>% count()
milieu_PID <- colData(sce) %>% as.data.frame %>% select(PID, milieu_tumor_6) %>% filter(!is.na(milieu_tumor_6)) %>% group_by(PID) %>% unique() %>% count()
...
ggplot(plot) + 
  geom_col(aes(y=spatial_CD8_pheno, x=fraction, fill=B_cell_score)) +
  theme_classic() + 
...
```{r}
sce <- minDistToCells(sce, img_id = "sample_id", x_cells = !is.na(sce$patch_B_cell), BPPARAM = MulticoreParam(workers = 10, stop.on.error = FALSE))
```
...
# Heatmap 
Heatmap(assay(spatialpheno_mean_sce, "exprs"), col = col_exprs, name = "exprs", cluster_columns = FALSE, row_names_side = "left", show_row_dend = FALSE)+
...
### First comparison: Differential interaction of tumor vs microenvironment between "node negative" vs. "node positive"
...
signifPlot(spicyTest,
           ...
           breaks=c(-3, 3, 1),
           fdr = TRUE,
           marksToPlot = unfactor(unique(cur_sce$metacluster_tumor_combined)))
...
sce <- minDistToCells(sce, img_id = "sample_id", x_cells = !is.na(sce$patch_B_cell), BPPARAM = MulticoreParam(workers = 10, stop.on.error = FALSE)) ## Add to B cell patch analysis part 

gc()
sce <- minDistToCells(sce, img_id = "sample_id", x_cells = !is.na(sce$patch_MC8), BPPARAM = MulticoreParam(workers = 10, stop.on.error = FALSE))
```
...
#also possible to use spatial_CD8_pheno_merg (cold/excluded vs inflamed)
cur_surv <- colData(sce) %>% as.data.frame() %>% select(PID, pT_merg_2, pN_merg, pT_simple, pM_simple,OS_months, DFS_months, status_DFS, status_OS) %>% unique()
...
abundances <- table(sce$cluster_combined, sce$PID)
...
abundances <- table(test_sce$metacluster_combined, test_sce$PID)
abundances <- unclass(abundances) 
head(abundances)

#relevant patient metadata (Relevel so that pT 1/2 is baseline)
meta <- colData(test_sce)[match(colnames(abundances), test_sce$PID),]
meta$spatial_CD8_pheno <- relevel(x=factor(meta$spatial_CD8_pheno), ref = "cold")

#filter
meta <- meta[!is.na(meta$spatial_CD8_pheno),]
abundances <- abundances[,colnames(abundances) %in% meta$PID]

#create DGEList - based on edgeR package
y.ab <- DGEList(abundances, samples=meta, group = meta$spatial_CD8_pheno)

#Design matrix
design <- model.matrix(~spatial_CD8_pheno, data = y.ab$samples)
...
Heatmap(cur_mat, 
        col = viridis(100),
        column_split = unfactor(meta$spatial_CD8_pheno))%v%
  columnAnnotation(spatial_CD8_pheno_merg = anno_simple(unfactor(meta$spatial_CD8_pheno)))
...
unique(sce$mixing_pheno)

sce$mixing_pheno_merg <- colData(sce) %>% 
  as.data.frame %>% 
  select(mixing_pheno) %>% 
  mutate(mixing_pheno_merg = ifelse(mixing_pheno %in% c("cold","mixed"),"cold_mixed", mixing_pheno)) %>% pull(mixing_pheno_merg)
...
#relevant patient metadata (Relevel so that pT 1/2 is baseline)
meta <- colData(sce)[match(colnames(abundances), sce$PID),]
meta$mixing_pheno_merg <- relevel(x=factor(meta$mixing_pheno_merg), ref = "cold_mixed")

#filter
meta <- meta[!is.na(meta$mixing_pheno_merg),]
abundances <- abundances[,colnames(abundances) %in% meta$PID]

#create DGEList - based on edgeR package
y.ab <- DGEList(abundances, samples=meta, group = meta$mixing_pheno_merg)

#Design matrix
design <- model.matrix(~mixing_pheno_merg, data = y.ab$samples)
...
Heatmap(cur_mat, 
        col = viridis(100),
        column_split = unfactor(meta$mixing_pheno_merg))%v%
  columnAnnotation(mixing_pheno_merg = anno_simple(unfactor(meta$mixing_pheno_merg)))
...
ggplot(DA, aes(cluster, y = logFC))+
  ...
  xlab("Cluster")+
  labs(fill = "FDR<0.1")+
  ggtitle("Spatial CD8 phenotype (cold/mixed vs compart)")+
  theme_classic()
...
#Abundance vs log fold change (best?)
diffabun_plot_mixing_1 <- ggplot(DA, aes(x = logFC, y = logCPM))+
  geom_point(aes(col= cluster, 
                 size = -log10(FDR)))+
  geom_label_repel(data = DA %>% filter(FDR<0.1), aes(label = cluster))+
  scale_color_manual(values = metadata(sce)$colors$metacluster_combined)+
  xlab("logFC (cold/mixed vs compartmentalized)")+
  ylab("Abundance (logCPM)")+
  theme_classic()+
  geom_vline(xintercept = 0, linetype = "dashed")+
  labs(color = "cluster")
...
#Abundance vs log fold change (best?)
diffabun_plot_mixing_2 <- ggplot(DA, aes(x = logFC, y = logCPM))+
  geom_point(aes(col= cluster, 
                 size = -log10(FDR)))+
  geom_label_repel(data = DA %>% filter(FDR<0.1), aes(label = cluster))+
  scale_color_manual(values = metadata(sce)$colors$cellular_neighborhood)+
