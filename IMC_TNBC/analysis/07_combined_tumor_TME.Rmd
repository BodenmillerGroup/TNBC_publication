...
dist_CD8_df$spatial_CD8_pheno <- sce$spatial_CD8_pheno[match(dist_df$sample_id, sce$sample_id)]
...
#also possible to use spatial_CD8_pheno_merg (cold/excluded vs inflamed)
cur_surv <- colData(sce) %>% as.data.frame() %>% filter(pT_merg_2 != "2") %>% 
  select(PID, pT_merg_2,OS_months, DFS_months, status_DFS, status_OS) %>% unique()

#Plot survival curves
fit <- survfit(Surv(OS_months, status_OS) ~ pT_merg_2, data = cur_surv)
print(fit)
...
fit <- survfit(Surv(DFS_months, status_DFS) ~ pT_merg_2, data = cur_surv)
print(fit)
