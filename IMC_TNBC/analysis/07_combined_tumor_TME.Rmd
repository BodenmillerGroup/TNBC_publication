...
# Select 13 clusters
set.seed(22)
cur_clusters <- kmeans(sce$aggregatedNeighbors, centers = 13, nstart = 10)$cluster
...
covariates <- Cox_df %>% select(-status_OS,-OS_months, -age, -grade, -pT_simple, -pN_simple, -pM_simple, -CN10) %>% colnames

multi_formula <- as.formula(paste("Surv(OS_months, status_OS) ~ age + grade + pT_simple + pN_simple + pM_simple +", paste(covariates, collapse = " + "),"+ CN10"))
...
#Based on proportions
covariates <- Cox_df %>% select(-status_DFS,-DFS_months, -age, -grade, -pT_simple, -pN_simple, -pM_simple, -CN10) %>% colnames

multi_formula <- as.formula(paste("Surv(DFS_months, status_DFS) ~ age + grade + pT_simple + pN_simple + pM_simple +", paste(covariates, collapse = " + "),"+ CN10"))
...
metadata(sce)$filterSpatialContext
...
# Select 8 clusters
set.seed(22)
tumor_cur_clusters <- kmeans(cur_dat, centers = 8, nstart = 100)$cluster
...
# Select 7 clusters
set.seed(22)
stroma_cur_clusters <- kmeans(cur_dat, centers = 7, nstart = 1000)$cluster
...
cur_sample <- colData(sce) %>% as.data.frame %>% group_by(spatial_metacommunity) %>% filter(spatial_metacommunity == "tumor_SMC_4") %>% count(sample_id) %>% arrange(desc(n)) %>% slice(1:15) %>% pull(sample_id)
...
cur_dat <- cur_dat %>% mutate(immune = B_cell+BnT_cell+CD4_T_cell+CD8_T_cell+HLA_DR+Monocyte+Neutrophil, .after = Neutrophil)
