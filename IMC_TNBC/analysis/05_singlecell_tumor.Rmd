...
PID_metacluster_p <- PID_metacluster %>% filter(Freq != 0) %>% group_by(PID) %>% count(name = "n_MC") %>% mutate(category = "PID")
...
PID_cluster_p <- PID_cluster %>% filter(Freq != 0) %>% group_by(PID) %>% count(name = "n_MC") %>% mutate(category = "PID")
