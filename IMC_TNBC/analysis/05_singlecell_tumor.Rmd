...
anno$metacluster <- as.factor((ifelse(anno$cluster_110 %in% row_order_ht[[i]], i,anno$metacluster)))
}
...
### Metacluster fraction comparison 

##1. by pT_merg
pT_cluster <- colData(sce) %>% as.data.frame() %>% select(PID,metacluster,pT_merg) %>% filter(!is.na(pT_merg)) %>% group_by(PID,metacluster, pT_merg) %>% count()
pT_cluster <- pT_cluster %>% group_by(PID) %>% mutate(fra = n/sum(n))

#Statistical testing
pT_cluster_kruskal <- compare_means(fra ~ pT_merg,pT_cluster, group.by = "metacluster", method = "kruskal.test", p.adjust.method = "BH")

pT_cluster_wil <- compare_means(fra ~ pT_merg,pT_cluster %>% filter(pT_merg %in% c("1","2")), group.by = "metacluster", method = "wilcox.test", p.adjust.method = "BH")
...
ggplot(pT_cluster, aes(x=pT_merg,y=fra))+
  ...
  theme_classic()+
  ylab("Patient cluster fraction")+
  xlab("pT_merg")+
  geom_text(pT_cluster_kruskal, mapping = aes(x = "1",y = 1, label = p.adj),inherit.aes = FALSE)

##2. by pN_merg
pN_cluster <- colData(sce) %>% as.data.frame() %>% select(PID,metacluster,pN_merg) %>% filter(!is.na(pN_merg)) %>% group_by(PID,metacluster, pN_merg) %>% count()
pN_cluster <- pN_cluster %>% group_by(PID) %>% mutate(fra = n/sum(n))

#Statistical testing
pN_cluster_kruskal <- compare_means(fra ~ pN_merg,pN_cluster, group.by = "metacluster", method = "kruskal.test", p.adjust.method = "BH")

pN_cluster_wil <- compare_means(fra ~ pN_merg,pN_cluster %>% filter(pN_merg %in% c("0","2/3")), group.by = "metacluster", method = "wilcox.test", p.adjust.method = "BH")
...
```{r OS survival_curves}
#Plot survival curves
fit <- survfit(Surv(DFS_months, status_DFS) ~ MC6, data = Cox_df_mean)
print(fit)
...
#GMM division
GMM_div <- Cox_df_GMM %>% select(colnames(Cox_df_GMM)[str_detect(colnames(Cox_df_GMM),"MC")]) %>% rownames_to_column("PID") %>% pivot_longer(cols = 2:8,names_to = "metacluster", values_to = "GMM")
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/05_tumor/TNBC_Tumor_Meta_HR_OS.pdf",width = 10,height = 8)
hr_prop/
hr_mean/
hr_GMM
dev.off()
