...
plot <- out_test %>% arrange(PID) %>% group_by(PID) %>% distinct(metacluster) %>% count(PID) %>% mutate(dominant = ifelse(n == "1", TRUE, FALSE)) %>% ungroup() %>% count(dominant) %>% mutate(fra = n/sum(n), category = "PID")

  
ggplot(plot, aes(x = category, y = n, fill = dominant))+
