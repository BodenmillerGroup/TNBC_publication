...
#retrieve row_order
ht_kmeans <- Heatmap(t(assay(cluster_mean_sce, "min_max")),
        ...
        show_row_names = T, 
        show_column_names =  T, 
        row_km = 12, row_km_repeats = 20000)
...
#Add color scheme for metaclusters
col_clusters$metaclusters <- col_clusters$cluster_100[order(as.numeric(names(col_clusters$cluster_100)))][1:12]
...
h <- Heatmap(t(assay(cluster_mean_sce, "min_max")),
  ...
  rowAnnotation(metaclusters = anno_simple(anno$metacluster,pch = anno$metacluster,border=TRUE,col_clusters$metaclusters))+
...
h_cluster_anno <- rowAnnotation(cluster_100 = anno_simple((row_order_clusters), pch = as.character(row_order_clusters),border=TRUE,col=col_clusters$cluster_100))+
  ...
  rowAnnotation(metaclusters = anno_simple(anno$metacluster[row_order_clusters],pch = anno$metacluster[row_order_clusters],border=TRUE,col_clusters$metaclusters))
...
UMAP_metaclus <- plotReducedDim(sce_sub, dimred = "UMAP", colour_by = "metacluster")+
  scale_color_manual(values = col_clusters$metaclusters)
...
SID_metacluster_cor_plot <- ggplot(SID_metacluster_cor, aes(x = category, y = V1))+
  ...
  theme_classic(base_size = 12)+
  theme(axis.title.x = element_blank())+
  ylab("Correlation of metacluster fractions per PID (n=2)")+
...
PID_groups <- PID_cluster %>% pivot_wider(id_cols = "PID",names_from = "cluster_100",values_from = "fra") %>% column_to_rownames("PID")
colnames(PID_groups) <- paste0("TC",colnames(PID_groups))


## 1. Based on continious cluster proportions
Cox_df <- cbind(PID_metagroups,PID_groups)
...
col_cluster_groups <- col_clusters$metaclusters[names(col_clusters$metaclusters) %in% col_anno$metacluster]


#Print heatmap
h_g_meta <- Heatmap(PID_groups, name = "Cluster fraction", 
        ...
        row_title_rot = 0)%v%
  columnAnnotation(metacluster = anno_simple(unfactor(col_anno$metacluster), pch = unfactor(col_anno$metacluster),border=TRUE, col = col_cluster_groups),annotation_name_side = "left") 
  
h_g_meta <- draw(h_g_meta)
draw(h_g_meta)
