...
cur_dat <- t(assay(sce[,sce$metacluster == "8"],"exprs")) %>% as.data.frame %>% select("CK5","CK7","CK8_18")

dim(cur_dat)
ggplot(cur_dat, aes(x=CK7, y=CK5)) +
  geom_point(alpha = 0.5)+
  stat_density_2d(aes(fill = ..level..), geom="polygon", contour_var = "count")+
  theme_classic()

ggplot(cur_dat, aes(x=CK8_18, y=CK5)) +
  geom_point(alpha = 0.5)+
  stat_density_2d(aes(fill = ..level..), geom="polygon", contour_var = "count",alpha = 0.1)+
  theme_classic()
```
...
## 4. Tumor heterogeneity analysis 
...
### Check whether heterogeneity of metaclusters influences survival 

shannon_plot$status_DFS <- sce$status_DFS[match(rownames(shannon_plot),sce$PID)]
shannon_plot$DFS_months <- sce$DFS_months[match(rownames(shannon_plot),sce$PID)]

### Plot survival curves
fit <- survfit(Surv(DFS_months, status_DFS) ~ heterogeneity_score, data = shannon_plot)
print(fit)
...
ggsurvplot(fit,
          ...
          palette = "Set1",
          log.rank.weights = "n")+
  ggtitle("Metacluster heterogeneity")
...
PID_cluster_p <- PID_cluster %>% filter(Freq > 5) %>% group_by(PID) %>% count(name = "n_MC") %>% mutate(category = "PID")

ggplot(PID_cluster_p, aes(x = category, y = n_MC))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = PID), size = 3, position=position_jitter(w=0.1,h=0.1))+
  guides(color = "none")+
  scale_color_manual(values = metadata(sce)$colors$PID)+
  ...
  theme(axis.title.x = element_blank())+
  ylab("Number of clusters per PID")


### Calculate shannon entropy per PID
library(vegan)

shannon_df <- PID_cluster %>% pivot_wider(id_cols = "PID",names_from = "cluster_110",values_from = "Freq") %>% column_to_rownames("PID")
...
ggplot(shannon_plot, aes(x = category, y = shannon_equi))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = PID), size = 3, position=position_jitter(w=0.1,h=0.1))+
  guides(color = "none")+
  scale_color_manual(values = metadata(sce)$colors$PID)+
  ...
  theme(axis.title.x = element_blank())+
  coord_cartesian(ylim = c(0,1))+
  ylab("Shannon entropy of tumor clusters per PID")

### Check whether heterogeneity of clusters influences survival 
...
out <- SID_metacluster_cor %>% filter(sample_id %in% i) %>% select(-Freq) %>% arrange(desc(fra)) %>% slice(1)
}
...
ggplot(out_bar, aes(x = category, y = n, fill = dominant))+
  geom_tile(position = "stack", alpha = 0.5)+
  theme_classic()+
  ylab("Number of PID")+
  scale_fill_brewer(palette = "Set1")
...
```{r}
plotSpatial(sce[,sce$CK5_7_group == "1"], 
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/05_tumor/TNBC_Tumor_CC_ClusterTesting.pdf",width = 10,height = 5)
p1+p2+p3
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/05_tumor/TNBC_Tumor_CC_Silhouette_Cluster110.pdf",width = 12,height = 10)
p4
dev.off()



pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/05_tumor/TNBC_Tumor_CC_PIDMetaCluster_ClusterCor.pdf",width = 12,height = 10)
(d_PID+SID_metacluster_cor_plot)/
(d_dens_PID+SID_metacluster_dens_cor_plot)
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/05_tumor/TNBC_Tumor_CC_PIDMetaCluster_CountHeatmap.pdf",width = 8,height = 6)
Heatmap(for_plot_MC, 
        name = "log(cell_count)",
        col = viridis(100)
        )
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/05_tumor/TNBC_Tumor_CC_PIDMetaClusterVSShannon.pdf",width = 4,height = 8)
n_MC_PID/shannon_MC_PID
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/05_tumor/TNBC_Tumor_Meta_HR_DFS.pdf",width = 8,height = 7)
hr_prop/
hr_mean
dev.off()
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/06_tumor/TNBC_Tumor_CC_ClusterMeta_HR_DFS.pdf",width = 8,height = 6)
hr_prop/
hr_cat/
hr_cat_GMM
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/06_tumor/TNBC_Tumor_CC_Heatmap_metaClus_PIDgroups.pdf",width = 8,height = 8)
h_g_meta
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/06_tumor/TNBC_Tumor_CC_Heatmap_Clus_PIDgroups.pdf",width = 8,height = 8)
h_g_clus
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/06_tumor/TNBC_Tumor_CC_HeatmapPIDgroups_mean.pdf",width = 11,height = 7)
h_g_rich
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/06_tumor/TNBC_Tumor_CC_HeatmapPIDgroupsCLR.pdf",width = 11,height = 7)
h_g_1
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/06_tumor/TNBC_Tumor_CC_HeatmapPIDgroupsCLR_mean.pdf",width = 11,height = 7)
h_g_rich_clr
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/06_tumor/TNBC_Tumor_CC_PIDgroups_Clus_OS.pdf",width = 8,height = 5)
plot_OS
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/06_tumor/TNBC_Tumor_CC_PIDgroups_Clus_DFS.pdf",width = 8,height = 5)
plot_DFS
dev.off()
