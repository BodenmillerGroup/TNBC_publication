...
PID_metacluster <- colData(sce) %>% as.data.frame() #%>% filter(pretreatment == FALSE) %>% select(metacluster,PID) %>% group_by(PID) %>% table() %>% as.data.frame
PID_metacluster <- PID_metacluster %>% group_by(PID) %>% mutate(fra = Freq/sum(Freq))
...
sce <- sce
abundances <- table(sce$CK5_7_score, sce$PID)
abundances <- unclass(abundances) 
head(abundances)
