...
##Visualize all mixed clusters on images
#run in loop
foreach(i = unique(sce_mixed$cluster_category_2))%do%{
#subset sce for critical clusters
sce_sub <- sce_mixed[,sce_mixed$cluster_category_2 == i]
#Prepare images
cur_images <- images[mcols(images)$sample_id %in% sce_sub$sample_id]
n <- ifelse(length(cur_images) < 20, length(cur_images), 20) #select maximum 20 images for time reasons
...
plotPixels(cur_images,
           mask = masks[mcols(masks)$sample_id %in% mcols(cur_images)$sample_id],
           object = sce_sub,
           img_id = "sample_id",
           ...
           save_plot = list(filename = paste0("/mnt/rcc_volume/TNBC/data_analysis/01_figures/mixed_clusters_viz/cluster_mixed_",i,".png"), scale = 2)
           )
}
...
Ecad_GMM = colData(sce) %>% as.data.frame %>% filter(GMM_Ecad == "Ecad+") %>% rownames_to_column() %>% pull("rowname"),
