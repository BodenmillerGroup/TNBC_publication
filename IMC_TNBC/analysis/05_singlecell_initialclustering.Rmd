...
anno_tumor <- fr_tum %>% rownames_to_column(var = "cluster") %>% mutate(cluster_category = ifelse(fr_tum > 0.8,"Epithelial",
                                                                                                  ifelse(fr_tum < 0.8 & fr_tum > 0.3,"Mixed","Non_Epithelial")))
...
h <- Heatmap(t(assay(mean_sce, "min_max")),
  ...
  Heatmap(spatial_norm,
        ...
        clustering_method_rows = "ward.D2",
        clustering_method_columns = "ward.D2")+
  rowAnnotation(Fraction_panCK = anno_barplot(fr_tum, width = unit(10, "mm"),gp = gpar(fill = "#377EB8", col = 1)),annotation_name_rot = 90)+
...
GMM_panCK = colData(sce) %>% as.data.frame %>% filter(GMM_CK == "panCK+") %>% rownames_to_column() %>% pull("rowname")
)
...
p6 <- plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "GMM_CK")+
  scale_color_brewer(palette="Set1")

p7 <- plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "cluster")+
  scale_color_manual(values = col_list_1[[1]])
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/TNBC_249_Tumor_HeatmapGMMCC.pdf",width = 13,height = 10)
draw(h)
dev.off()
