...
sce_spill <- readSCEfromTXT("/Volumes/rcc_volume/TNBC/spillover/spillover_renamed/")
...
sm_jana <- read.csv("~/Desktop/sm_ZTMA21_26.csv",row.names = 1)
...
```{r write_sm_csv}
sm
write.csv(sm, "/Volumes/rcc_volume/TNBC/data_analysis/spillover_matrix_LM")

sm_jana
write.csv(sm, "/Volumes/rcc_volume/TNBC/data_analysis/spillover_matrix_Jana")
```
...
panel <- read.csv("/Volumes/rcc_volume/TNBC/panel.csv")
adapted_sm <- adaptSpillmat(sm_jana, paste0(panel$channel[panel$keep == 1], "Di"),
                            isotope_list = isotope_list)
?compImage

images_comp <- compImage(images, adapted_sm, BPPARAM = MulticoreParam(n=6))
```
