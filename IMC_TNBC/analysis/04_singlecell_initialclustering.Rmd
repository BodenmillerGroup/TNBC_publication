...
```{r clus_mixed, message=FALSE, warning=FALSE, fig.width=10, fig.height=12}
...
#here total frequency of tumor cells per sample > 30 (100?)
SID_select_tumor_content <- tumor_cell_SID %>% filter(Freq < 30) %>% pull(sample_id) %>% unfactor 

sce$image_low_tumor <- sce$sample_id %in% SID_select_tumor_content
```
