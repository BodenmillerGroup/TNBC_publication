...
Here, we again pre-select 10 images of interest (e.g. high immune infiltration, TLS, high epithelial cell number). We then perform image- and channel-wise
...
plotPixels(cur_images,
           ...
           #save_plot = list(filename = "/Users/lasmey/Desktop/TNBC/Figures/Masks_comparison/TNBC_masks_epi_4_spilcor.png", scale = 2))
...
plotPixels(cur_images,
           ...
           legend = list(colour_by.title.cex = 0.7,
                         colour_by.labels.cex = 0.7))
           #save_plot = list(filename = "/Users/lasmey/Desktop/TNBC/Figures/Masks_comparison/TNBC_masks_immune_4_spilcor.png", scale = 2))
```
...
#Remove split image aquisitions
...
```{r stainingarti, message=FALSE, warning=FALSE}
#Randomly select 5 sample Ids for visualization
set.seed(22)
sampleid_random <- sample(unique(sce$sample_id), 5)

multi_dittoPlot(sce[,sce$sample_id %in% sampleid_random], vars = rownames(sce)[rowData(sce)$use_channel],
...
Finally, we will use non-linear dimensionality reduction methods to project cells from a high-dimensional (40) down to a low-dimensional (2) space. For this the scater package provides the runUMAP (and runTSNE) function. To ensure reproducibility, we will set a seed. 

**Goal:** Visualization of the low-dimensional embedding facilitates assessment of potential "batch effects".
