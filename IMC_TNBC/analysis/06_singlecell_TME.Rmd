...
astir_col <- setNames(c("#2E81B9","#91C893","#5DB54B","#A29C68","#FDB056","#CAB2D6","white"),sort(unique(sce$astir_TME)))
...
cluster_col <- setNames(colorRampPalette(brewer.pal(9, "Paired"))(length(unique(anno$cluster_130))), row_order_clusters)                                        
metacluster_col <- setNames(colorRampPalette(brewer.pal(9, "Paired"))(length(unique(anno$metacluster))), sort(unique(anno$metacluster)))                                          
...
dittoBarPlot(sce, var = "immune_metacluster", group.by = "PID")+
  scale_fill_manual(values = metadata(sce)$colors$immune_metacluster)+
  theme(axis.text.x = element_blank())
