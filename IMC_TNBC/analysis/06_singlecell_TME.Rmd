...
#based on cluster categories (n=436281)
...
other_epi_channel = c("Vimentin", "Fibronectin","panCK","Ecad","CK5","CK8_18","KRT14","cMYC","HER2","ER","PR","p53","AR","EGFR","p_mTOR","CK7","GATA3","CD45")
...
saveRDS(sce, "/mnt/rcc_volume/TNBC/data_analysis/06_sce_TNBC_TME.rds")
...
#Cluster colors 
col_clus <- colorRampPalette(brewer.pal(9,"Paired"))(length(unique(sce$cluster_130)))
...
## Optional: Cluster merging of similar clusters with clear phenotype
...
anno_clus <- as.character(sce$cluster_130)

anno_clus[anno_clus == "1"] <- "FibroVim_SMA+"
anno_clus[anno_clus == "2"] <- "FibroVim"
anno_clus[anno_clus == "3"] <- "Fibro"
...
anno_clus[anno_clus == "13"] <- "BnT_cells"
anno_clus[anno_clus == "14"] <- "FibroSMA"

anno_clus <- data.frame(anno_clus = anno_clus, origin = as.character(sce$cluster_130))

anno$names <- anno_clus$anno_clus[match(anno$cluster_130,anno_clus$origin)]
sce$cluster_names <- anno_clus$anno_clus[match(sce$cluster_130,anno_clus$origin)]
...
cluster_SID <- colData(sce) %>% as.data.frame() %>% select(cluster_130,sample_id) %>% group_by(cluster_130) %>% table() %>% as.data.frame
n_SID <- cluster_SID %>% filter(Freq>0) %>% group_by(cluster_130) %>% count(name = "n_SID") %>% column_to_rownames("cluster_130")
...
cluster_CC <- colData(sce) %>% as.data.frame() %>% select(cluster_130,cluster_category) %>% group_by(cluster_130) %>% table() %>% as.data.frame
cluster_CC <- cluster_CC %>% group_by(cluster_130) %>% mutate(fra = Freq/sum(Freq)) 
cluster_CC <- cluster_CC %>% select(-Freq) %>% pivot_wider(id_cols = cluster_130,names_from = cluster_category,values_from = fra) %>% column_to_rownames("cluster_130")
...
col <- list(cluster_130 = colorRampPalette(brewer.pal(12, "Accent"))(length(unique(anno$cluster_130))))
col_clusters <- col[[1]]
names(col_clusters) = unique(anno$cluster_130)[row_order_clusters]
col_clusters <- list(col_clusters)
names(col_clusters) <- "cluster_130"
...
h <-Heatmap(t(assay(cluster_mean_sce, "min_max")),
  ...
  Heatmap(t(assay(other_mean_sce, "min_max")),
        ...
        clustering_method_columns = "ward.D2")+
  ha_spatial+
  rowAnnotation(cluster_130 = anno_simple(unfactor(anno$cluster_130), pch = unfactor(anno$cluster_130),border=TRUE,col=col_clusters$cluster_130))+
...
h_cluster_anno <- rowAnnotation(cluster_130 = anno_simple(unfactor((unique(anno$cluster_130)[row_order_clusters])), pch = as.character(unfactor((unique(anno$cluster_130)[row_order_clusters]))),border=TRUE,col=col_clusters$cluster_130))+
...
## Cox-proportonal hazards model for TME phenotypes
...
### Univariate cox proportional hazards model on multiple covariates (here proportion of phenotypes)

PID_groups <- PID_cluster %>% pivot_wider(id_cols = "PID",names_from = "cluster_50_renamed",values_from = "fra") %>% column_to_rownames("PID")
colnames(PID_groups) <- paste0("TME",colnames(PID_groups))
...
covariates <- Cox_df %>% select(-status_OS,-OS_months) %>% colnames
univ_formulas <- sapply(covariates, function(x) as.formula(paste('Surv(OS_months, status_OS)~', x)))
...
univ_results <- lapply(univ_models,
                       function(x){ 
                          ...
                          names(res)<-c("beta", "HR", "CI_low","CI_high","wald_test", 
                                        "p_value")
                          return(res)
                         })
...
res <- res %>% rownames_to_column(var = "group")

#Plot hazard ratios            
hr_prop <- ggplot(res, aes(x = reorder(group,desc(HR)), y = HR, color = p_value < 0.05))+
geom_point(size = 4)+
...
Cox_df_mean$status_OS <- sce$status_OS[match(rownames(Cox_df_mean),sce$PID)]
Cox_df_mean$OS_months <- sce$OS_months[match(rownames(Cox_df_mean),sce$PID)]

#Based on proportions
covariates <- Cox_df_mean %>% select(-status_OS,-OS_months) %>% colnames
univ_formulas <- sapply(covariates, function(x) as.formula(paste('Surv(OS_months, status_OS)~', x)))
univ_models <- lapply(univ_formulas, function(x){coxph(x, data = Cox_df_mean)})
...
res_1 <- t(as.data.frame(univ_results, check.names = FALSE))
res_1 <- as.data.frame(res_1)
res_1 <- res_1 %>% rownames_to_column(var = "group")

#Plot hazard ratios            
hr_cat <- ggplot(res_1, aes(x = reorder(group,desc(HR)), y = HR, color = p_value < 0.05))+
geom_point(size = 4)+
geom_errorbar(aes(ymax = CI_high, ymin = CI_low))+
...
scale_color_brewer(palette = "Set1")+
xlab("TME phenotypes")+
ylab("HR for mean_prop")+
theme_classic()+
theme(axis.text.x = element_text(angle = 90))


##3. Based on categories (GMM division of cluster proportions)
Cox_df_GMM <- foreach(i = Cox_df %>% select(-status_OS,-OS_months) %>% colnames,.combine = "cbind")%do%{
GMM_Cox <- Mclust(Cox_df[,i], G = 2)
GMM_Cox$classification
}
colnames(Cox_df_GMM) <- Cox_df %>% select(-status_OS,-OS_months) %>% colnames
rownames(Cox_df_GMM) <- Cox_df %>% select(-status_OS,-OS_months) %>% rownames
Cox_df_GMM <- as.data.frame(Cox_df_GMM)
Cox_df_GMM$status_OS <- sce$status_OS[match(rownames(Cox_df_GMM),sce$PID)]
Cox_df_GMM$OS_months <- sce$OS_months[match(rownames(Cox_df_GMM),sce$PID)]

#Based on proportions
covariates <- Cox_df_GMM %>% select(-status_OS,-OS_months) %>% colnames
univ_formulas <- sapply(covariates, function(x) as.formula(paste('Surv(OS_months, status_OS)~', x)))
univ_models <- lapply(univ_formulas, function(x){coxph(x, data = Cox_df_GMM)})
...
res_2 <- t(as.data.frame(univ_results, check.names = FALSE))
res_2 <- as.data.frame(res_2)
res_2 <- res_2 %>% rownames_to_column(var = "group")

#Plot hazard ratios            
hr_cat_GMM <- ggplot(res_2, aes(x = reorder(group,desc(HR)), y = HR, color = p_value < 0.05))+
geom_point(size = 4)+
geom_errorbar(aes(ymax = CI_high, ymin = CI_low))+
coord_cartesian(ylim=c(0,10))+
geom_hline(yintercept = 1,color = "black",linetype = "dashed")+
...
#currently too many covariates and too little observations: ratio should be 1:10
```
### For DFS
...
univ_formulas <- sapply(covariates, function(x) as.formula(paste('Surv(DFS_months, status_DFS)~', x)))
univ_models <- lapply(univ_formulas, function(x){coxph(x, data = Cox_df)})
...
##3. Based on categories (GMM division of cluster proportions)
Cox_df_GMM <- foreach(i = Cox_df %>% select(-status_DFS,-DFS_months) %>% colnames,.combine = "cbind")%do%{
GMM_Cox <- Mclust(Cox_df[,i], G = 2)
GMM_Cox$classification
}
colnames(Cox_df_GMM) <- Cox_df %>% select(-status_DFS,-DFS_months) %>% colnames
rownames(Cox_df_GMM) <- Cox_df %>% select(-status_DFS,-DFS_months) %>% rownames
Cox_df_GMM <- as.data.frame(Cox_df_GMM)
Cox_df_GMM$status_DFS <- sce$status_DFS[match(rownames(Cox_df_GMM),sce$PID)]
Cox_df_GMM$DFS_months <- sce$DFS_months[match(rownames(Cox_df_GMM),sce$PID)]

#Based on proportions
covariates <- Cox_df_GMM %>% select(-status_DFS,-DFS_months) %>% colnames
univ_formulas <- sapply(covariates, function(x) as.formula(paste('Surv(DFS_months, status_DFS)~', x)))
univ_models <- lapply(univ_formulas, function(x){coxph(x, data = Cox_df_GMM)})
...
plotPixels(cur_images,
           ...
           save_plot = list(filename = paste0("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TME_clusters_viz/TME_pixel_clusterbeforemerg_Bcells.png"), scale = 2)
           )
...
plotPixels(cur_images,
           ...
           legend = list(colour_by.title.cex = 0.7,
                         colour_by.labels.cex = 0.7),
           save_plot = list(filename = paste0("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TME_clusters_viz/TME_pixel_cluster_EndoCells.png"), scale = 2)
           )
...
plotPixels(cur_images,
           ...
           legend = list(colour_by.title.cex = 0.7,
                         colour_by.labels.cex = 0.7),
           save_plot = list(filename = paste0("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TME_clusters_viz/TME_pixel_cluster_NeutroGranu.png"), scale = 2)
           )
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_ClusterTesting.pdf",width = 10,height = 5)
p1+p2+p3
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_Silhouette_Cluster50.pdf",width = 12,height = 10)
p4
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_HeatmapCluster.pdf",width = 13,height = 7)
h
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_HeatmapCluster_Scales.pdf",width = 13,height = 8)
h_exprs + h_min_max
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_ClusterAnno.pdf",width = 5,height = 8)
h_cluster_anno
dev.off()


pdf("/Volumes/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_HeatmapCluster_subcells.pdf",width = 10,height = 8)
dittoHeatmap(sce_sub, 
             genes = rownames(sce_sub)[rowData(sce_sub)$cluster_channel],
             assay = "min_max", 
             scale = "none", 
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_ExpressionCor.pdf",width = 10,height = 8)
corrplot(cor(cor_sce,method = "spearman"), method="color", col=brewer.pal(n=8, name="RdBu"),  
         type="upper", order="hclust",hclust.method = "ward.D2", 
         addCoef.col = "black", addCoefasPercent = F,# Add coefficient of correlation
         tl.col="black", tl.srt=40, #Text label color and rotation
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_TSNECluster.pdf",width = 8,height = 6)
TSNE_clus
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_TSNEExpression.pdf",width = 12,height = 6)
exp_plots_TSNE
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_UMAPExpression.pdf",width = 12,height = 6)
exp_plots_UMAP
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_ClusterPIDSID.pdf",width = 12,height = 6)
p_clusterPID
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_PIDCluster_ClusterCor.pdf",width = 14,height = 6)
p_PID_cluster+SID_cluster_cor_plot
dev.off()

pdf("/Volumes/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_HeatmapPIDgroups.pdf",width = 11,height = 7)
h_g
dev.off()

pdf("/Volumes/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_HeatmapPIDgroupsCLR.pdf",width = 11,height = 7)
h_g_1
dev.off()

pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/07_TME/TNBC_249_TME_CC_Cluster_HR_DFS.pdf",width = 8,height = 10)
hr_prop/
hr_cat/
hr_cat_GMM
dev.off()
