...
## Add colors 
metadata(vsd)$colors$KRT57_score <- setNames(c("grey","orange","firebrick","green4"), c("KRT5/7-", "KRT5/7+", "KRT5+", "KRT7+"))
...
keratins <- c("KRT5","KRT7","KRT15","KRT8","KRT18","KRT19", "KRT14","KRT17")
...
## Option 2: vst counts
df <- t(assay(vsd)[keratins,]) %>% as.data.frame %>% rownames_to_column("PID")

## Create heatmap body 
df$KRT5_7_score <- vsd$KRT5_7_score[match(df$PID, colnames(vsd))]

mat <- df %>% pivot_longer(cols = 2:9, names_to = "gene", values_to = "exprs") %>% group_by(KRT5_7_score, gene) %>% summarize(mean = mean(exprs))

ht <- mat %>% pivot_wider(names_from = "gene", values_from = "mean") %>% column_to_rownames("KRT5_7_score")

## Print heatmap 
Heatmap(ht, name = "vst counts",col = viridis(100))

Heatmap(scale(ht), name = "scaled vst counts")
```
