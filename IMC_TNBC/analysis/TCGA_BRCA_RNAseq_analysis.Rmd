...
```{r}
colData(vsd)$age <- as.numeric(information.subtype.xena$Age_at_Initial_Pathologic_Diagnosis_nature2012[match(colData(brca.clean)[,"PID"],vsd$PID)])
```
...
sig.up <- res %>% as.data.frame %>% filter(log2FoldChange > 1) %>% arrange(padj) %>% slice(1:25) %>% rownames_to_column("gene") %>% pull(gene)
sig.down <- res %>% as.data.frame %>% filter(log2FoldChange < -1) %>% arrange(padj) %>% slice(1:25) %>% rownames_to_column("gene") %>% pull(gene)
...
# Plot Heatmap
Heatmap(mat_up, column_split = anno$KRT5_7_score_combined, column_title = NULL)%v%
...
Heatmap(mat_down, column_split = anno$KRT5_7_score_combined, column_title = NULL)%v%
  columnAnnotation(KRT5_7_score_combined = anno$KRT5_7_score_combined)%v%
  columnAnnotation(KRT5_7_score = anno$KRT5_7_score)%v%
  columnAnnotation(PAM50 = anno$PAM50)%v%
  columnAnnotation(TNBC = anno$TNBC)
...
plot_vol <- reshape %>% as.data.frame %>% rownames_to_column("gene")
plot_vol <- res %>% as.data.frame %>% rownames_to_column("gene")
...
```{r}
p1 <- ggplot(anno, aes(x = KRT5_7_score_combined, y = MUC4))+
...
p2 <- ggplot(anno, aes(x = KRT5_7_score, y = MUC4))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = KRT5_7_score_combined), size = 3, position=position_jitter(w=0.1,h=0.1))+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90))+
...
p3 <- ggplot(anno, aes(x = TNBC, y = MUC4))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = KRT5_7_score_combined), size = 3, position=position_jitter(w=0.1,h=0.1))+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90))+
...
gseaplot2(gse, title = gse$Description[16], geneSetID = 1)
```
...
# Run KEGG
gseKEGG <- gseKEGG(geneList=gene_list_kegg_unique, 
        organism = "hsa",
        keyType = "kegg", 
        minGSSize = 10, 
        maxGSSize = 500, 
...
## EMAP plot 
emapplot(pairwise_termsim(gseKEGG), showCategory = 15)
...
rownames(assay(brca.clean, "tpm_unstrand"))
...
ggplot(res_MCP_plot, aes(x = TNBC, y = score))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = TNBC), size = 3, position=position_jitter(w=0.1,h=0.1))+
