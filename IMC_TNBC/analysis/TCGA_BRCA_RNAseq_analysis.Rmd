...
mat <- df %>% pivot_longer(cols = 2:11, names_to = "gene", values_to = "exprs") %>% group_by(KRT5_7_score, gene) %>% summarize(mean = mean(exprs))
...
```{r}
# Define keratins of interest
stem <- c("SOX9","KIT","PROM1","YAP1","HDAC2","NOTCH1","TNF","NFKB1","CD44","ALDH1A1","BRCA1","BRCA2")
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/TCGA_BRCA/TCGA_BRCA_CK57score_Heatmap_Stem.pdf", height = 4, width = 7, onefile = FALSE)
Heatmap(scale(ht), name = "Scaled mean Z-scores")
dev.off()
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/TCGA_BRCA/TCGA_BRCA_CK57score_Basal_exprs.pdf", height = 5, width = 5, onefile = FALSE)
ggplot(df, aes(x = KRT5_7_score, y = signature_sum))+
  geom_boxplot(outlier.color = NA)+
  geom_jitter(aes(color = KRT5_7_score), size = 3, position=position_jitter(w=0.1,h=0.1))+
  scale_color_manual(values = metadata(vsd)$colors$KRT57_score)+
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/TCGA_BRCA/TCGA_BRCA_CK57score_Basal_Heatmap.pdf", height = 4, width = 7, onefile = FALSE)
Heatmap(ht, name = "Mean signature Z-scores", show_column_names = FALSE)
dev.off()
