...
Idea: Cluster TCGA BRCA patients based on informative markers from IMC analysis - CK5/7/8/14/18/CD8a/HLADR - to potentially find the same PID groups
...
GOI <- c("KRT5","KRT14","KRT7","KRT8","KRT18","CD8A","HLA-DRA","KRT19") #KRT19 
# <- c("KRT5","KRT7","CD8A")
...
#k=20
out <- Rphenoannoy(mat, k = 20)

clusters <- factor(membership(out[[2]]))

vsd$cluster_20 <- clusters

unique(vsd$cluster_20)
...
vsd$cluster_30 <- clusters

unique(vsd$cluster_30)
...
vsd$cluster_50 <- clusters

unique(vsd$cluster_50)
...
vsd$cluster_70 <- clusters

unique(vsd$cluster_70)
...
vsd$cluster_90 <- clusters

unique(vsd$cluster_90)
...
assayNames(vsd) <- "vst"
clusters <- colnames(colData(vsd))[str_detect(colnames(colData(vsd)),"cluster_[:digit:]")]
df <- foreach(i = seq_along(clusters),.combine = rbind) %do% {
num_clus <- length(unique(colData(vsd)[,clusters[i]]))
sil <- approxSilhouette(t(assay(vsd, "vst")), clusters=colData(vsd)[,clusters[i]])
#purity <- neighborPurity(t(assay(vsd, "exprs")), clusters=colData(vsd)[,clusters[i]]) #takes very long to compute - do once and then save figure
rmsd <- clusterRMSD(t(assay(vsd, "vst")), clusters=colData(vsd)[,clusters[i]])
...
```{r}
GOI <- c("KRT5","KRT14","KRT7","KRT8","KRT18","CD8A","HLA-DRA","KRT19", "CD274") #CD274 == PDL1

assayNames(vsd) <- "vst"

cluster_mean_sce <- aggregateAcrossCells(vsd, ids = vsd$PID_group_test,
                                 statistics = "mean",
                                 use.assay.type = "vst",
                                 subset.row = rowData(vsd)$gene_name %in% GOI)

Heatmap(t(assay(cluster_mean_sce, "vst")), col = viridis(100))
Heatmap(scale(t(assay(cluster_mean_sce, "vst"))))
```
...
cur_surv <- colData(vsd) %>% as.data.frame() %>% #filter(TNBC == "TNBC") %>% 
  ...
  dplyr::select(PID, PID_group_test, DFI, DFI.time, OS, OS.time, DSS, DSS.time, PFI, PFI.time) #%>% filter(cluster_90 %in% c("1","5"))
...
```{r}
dittoBarPlot(vsd, var = "PID_group_test", group.by = "TNBC")
dittoBarPlot(vsd, var = "PID_group_test", group.by = "TNBC", scale = "count")


dittoBarPlot(vsd, var = "PID_group_test", group.by = "PAM50Call_RNAseq")
dittoBarPlot(vsd, var = "PID_group_test", group.by = "PAM50Call_RNAseq", scale = "count")
```
...
GMM_df <- data.frame(class_epi = ifelse(GMM_CD8$classification == 2 & GMM_CD8$uncertainty < 0.00001, 
                                        "HLADR+",
                                        "HLADR-"), 
