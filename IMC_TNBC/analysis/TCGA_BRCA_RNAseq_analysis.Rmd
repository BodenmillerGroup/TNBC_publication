...
GOI <- c("KRT5","KRT14","KRT7","KRT8","KRT18","CD8A","HLA-DRA") #KRT19 
#GOI <- c("KRT5","KRT7","CD8A")
...
cluster_mean_sce <- aggregateAcrossCells(vsd, ids = vsd$cluster_90,
                                 statistics = "mean",
                                 use.assay.type = "vst",
...
# Note to self: Groups 1/5 could represent inflamed/BL? 
```
...
cur_surv <- colData(vsd) %>% as.data.frame() #%>% filter(PAM50Call_RNAseq == "Basal") %>% 
  dplyr::select(PID, cluster_90, DFI, DFI.time, OS, OS.time, DSS, DSS.time, PFI, PFI.time) #%>% filter(cluster_90 %in% c("1","5"))


#Plot survival curves
fit <- survfit(Surv(DFI.time, DFI) ~ cluster_90, data = cur_surv)
print(fit)
...
#Plot survival curves
fit_1 <- survfit(Surv(PFI.time, PFI) ~ cluster_90, data = cur_surv)
print(fit_1)
...
```{r}
dittoBarPlot(vsd, var = "cluster_90", group.by = "TNBC")
dittoBarPlot(vsd, var = "cluster_90", group.by = "TNBC", scale = "count")


dittoBarPlot(vsd, var = "cluster_90", group.by = "PAM50Call_RNAseq")
dittoBarPlot(vsd, var = "cluster_90", group.by = "PAM50Call_RNAseq", scale = "count")
```
