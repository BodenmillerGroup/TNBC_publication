...
```{r}
# read in data 
dat <- read_csv("/mnt/rcc_volume/TNBC/data_analysis/03_data/QuPath_Basoluminal_Quantification_Annotation_4.csv")
...
```{r}
write.csv(cur_IF_1, "/mnt/rcc_volume/TNBC/data_analysis/03_data/TNBC_IF_run_1celltype_fractions.csv", row.names = TRUE)
```
...
cur_out <- cur_dat %>% filter(CK_group %in% c("CK5:CK19:SOX9", "CK5:CK19")) %>% group_by(PID, CK_group) %>% summarise(mean_n = mean(n)) %>% #Average images per PID 
  group_by(PID) %>% mutate(sum = sum(mean_n)) %>% mutate(fra = mean_n/sum) %>% filter(CK_group == "CK5:CK19:SOX9") %>% select(PID, CK_group, fra, mean_n) %>% unique()
...
spearman_test <- cor.test(cur_out$fra,cur_out$mean_n, method = "spearman")

ggplot(cur_cor, aes(x = fra, y = mean_n))+
  geom_smooth(method='lm', formula= y~x, se = TRUE, color = "darkgrey")+
...
cur_out$CK_subgroup <- factor(cur_out$CK_subgroup, levels = c("BL","B","L"))
...
### Correlation IF run1/run2 

```{r}
cur_IF_1 <- read.csv("/mnt/rcc_volume/TNBC/data_analysis/03_data/TNBC_IF_run_1celltype_fractions.csv",row.names = 1)

cur_IF_1
cur_IF_2 #IF Basolum (Run 2)

cur_IF_1 <- cur_IF_1[match(rownames(cur_IF_2), rownames(cur_IF_1)),] #IF Basolum - Stem (Run 2)
cur_IF_1 <- cur_IF_1[,match(colnames(cur_IF_2), colnames(cur_IF_1))] #IF Basolum - Stem (Run 2)

paste0("BL:", cor(cur_IF_1$BL, cur_IF_2$BL), " B:", cor(cur_IF_1$B, cur_IF_2$B), " L:", cor(cur_IF_1$L, cur_IF_2$L))
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/IF_Basoluminal/TNBC_QuPath_Basoluminal_Heatmap_CK57.pdf",height = 4, width = 10, onefile = FALSE)
draw(h_1,annotation_legend_list = list(lgd_1))
dev.off()
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/IF_Basoluminal/TNBC_QuPath_Basoluminal_CK7_19.pdf", height = 5, width = 4)
ggplot(cur_719, aes(x=CK7_19_group, y=fra))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = PID), size = 3, position=position_jitter(w=0.1,h=0.1))+
  scale_color_manual(values = metadata(sce)$colors$PID)+
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/IF_Basoluminal/TNBC_QuPath_Basoluminal_BL_SOX9_Boxplot.pdf", height = 5, width = 4)
ggplot(cur_out, aes(x=CK_group, y=fra))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = PID), size = 3, position=position_jitter(w=0.1,h=0.1))+
  scale_color_manual(values = metadata(sce)$colors$PID)+
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/IF_Basoluminal/TNBC_QuPath_Basoluminal_AllGroups_SOX9_Boxplot.pdf", height = 5, width = 5)
ggplot(cur_out, aes(x=CK_subgroup, y=fra))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = PID), size = 3, position=position_jitter(w=0.1,h=0.1))+
  scale_color_manual(values = metadata(sce)$colors$PID)+
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/IF_Basoluminal/TNBC_QuPath_Basoluminal_IF1_2_Correlation.pdf", height = 5, width = 4)
corrplot(cor(cur_IF_1, cur_IF_2,method = "spearman"), method="color", col=brewer.pal(n=8, name="RdBu"),  
         type="upper", order="hclust",hclust.method = "ward.D2", 
         addCoef.col = "black", addCoefasPercent = F,
         tl.col="black", tl.srt=40,
