...
Next, we will run Rphenograph (Jaccard-based weights, Louvain clustering as community detection algorithm) clustering with different resolution parameters (k=50,75,100,125). 
...
#Double-check 
p5 <- plotSpatial(sce[,colData(sce)$sample_id == "ZTMA249.1_ZTMA249_ID_1_AX1Y1_93"],img_id = "sample_id",colPairName = "neighborhood",draw_edges = FALSE, node_color_by = "n_neighbors",node_size_fix = 2)+
  scale_color_continuous(type = "viridis")+
  theme_classic()
p6 <- plotSpatial(sce[,colData(sce)$sample_id == "ZTMA249.1_ZTMA249_ID_1_AX1Y1_93"],img_id = "sample_id",colPairName = "neighborhood",draw_edges = TRUE, node_color_by = "GMM_CK")+
...
cluster_names <- c("CK5/7+","p53/EGFR/Ki67+","Vim/KRT14+","CK5low","Vim+","Unclear","Unclear","Ki67/p53+","Vim/Ki67+","Unclear","Her2/KRT14+","CD15+","Vim/Ki67/p53+","CK7+","Vim/Her2/CK7+","Vim/Ki67+_2","Vim/p53+","GATA3low","HER2+","CK7/8/18+","Vim/CK5+","ER/GATA3+","KRT14+","KRT14/Ki67+","p53/cMYC+")
...
h <-Heatmap(t(assay(cluster_mean_sce, "min_max")),
  ...
  Heatmap(spatial_norm,
        ...
        clustering_method_rows = "ward.D2",
        clustering_method_columns = "ward.D2")+
  rowAnnotation(cluster_100 = anno$cluster_100,
...
```{r pgroups, eval = F}
PID_cluster
## Aim: Cluster patients based on tumor cluster fractions

#kick out unclear clustering with little information (here 6 clusters)"
PID_groups <- PID_cluster %>% #filter(!cluster_100 %in% c("4","6","7","10","14","18")) %>% 
...
meta_PID_groups <- PID_groups %>% rownames_to_column(var = "PID")


### Heatmap of TNBC patient groups
...
library(clValid)
#Internal measures
testCL_1<- clValid(main,nClust = 3:25,clMethods = c("hierarchical","agnes","pam","kmeans"),validation="internal",metric="euclidean", method = "ward")
summary(testCL_1)
plot(testCL_1, legend = FALSE)

#Plot dendogram for hierarchical clustering
#library(dendextend)
#avg_dend_obj_MCP <- as.dendrogram(agnes(main),metric = "euclidean",method = "ward")
#avg_col_dend_MCP <- color_branches(avg_dend_obj_MCP, k = 19)
#plot(avg_col_dend_MCP)

#Print heatmap
Heatmap(main, name = "Cluster fraction", col = col_main,km = 21,row_km_repeats = 1000,
            show_column_names = T, show_row_names =  F, clustering_method_rows = "ward.D2",
            clustering_method_columns = "ward.D2")
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/TNBC_249_Tumor_CC_ClusterTesting.pdf",width = 10,height = 5)
p1+p2+p3
dev.off()
...
pdf("/mnt/rcc_volume/TNBC/data_analysis/01_figures/TNBC_249_Tumor_CC_HeatmapCluster.pdf",width = 12,height = 8)
h
dev.off()
