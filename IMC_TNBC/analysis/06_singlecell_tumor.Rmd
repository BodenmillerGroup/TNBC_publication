...
#based on cluster categories (n=315070)
...
anno_clus[anno_clus == "22"] <- "HER2+/CK8/18low"
...
# Proportion of lehmann subtype per cluster
...
#Quality control: Check number of tumor cells per sample/PID
```{r n tumor cells, message = FALSE, warning=FALSE}
n_tumor_SID <- cluster_SID %>% ungroup %>% group_by(sample_id) %>% mutate(sum = sum(Freq)) %>% select(sample_id,sum) %>% unique() %>% arrange(sum)

ggplot(n_tumor_SID) +
  geom_point(aes(reorder(sample_id,sum), sum, color = sum))+
  geom_hline(yintercept = median(n_tumor_SID$sum), linetype="dashed", color = "grey")+
  geom_hline(yintercept = quantile(n_tumor_SID$sum)[2],linetype="dashed", color = "grey")+
  geom_hline(yintercept = quantile(n_tumor_SID$sum)[4],linetype="dashed", color = "grey")+
  theme_classic(base_size = 15)+
  theme(axis.text.x = element_blank())+
  ylab("Number of tumor cells per sample")+ 
  xlab("")
...
#e.g. >=20 cells per sample or >=30 cells per PID
```
...
PID_groups <- PID_cluster %>% #filter(!cluster_100 %in% c("26","23","1","22","14","5","6","7","16","19","11")) %>% 
...
avg_col_dend_MCP <- color_branches(avg_dend_obj_MCP, k = 14)
plot(avg_col_dend_MCP)
...
coldata_sce <- colData(sce) %>% 
  ...
  distinct() %>% 
  filter(!PID_groups == "1") #remove mixed/undefined group
