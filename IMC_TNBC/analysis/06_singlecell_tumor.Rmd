...
library(clValid)
#Internal measures
testCL_1<- clValid(PID_groups,nClust = 3:20,clMethods = c("hierarchical","agnes","kmeans"),validation="internal",metric="euclidean", method = "ward")
summary(testCL_1)
plot(testCL_1, legend = FALSE)

out_k <- kmeans(PID_groups,14,nstart = 1000,iter.max = 1000)
...
ha_right  <- rowAnnotation(PID_groups = anno_simple(row_anno$PID_groups,border=TRUE))




set.seed(22)
<- Heatmap(PID_groups, name = "Cluster fraction", col = col_main,km = 14,row_km_repeats = 1000,
            show_column_names = T, show_row_names =  F, clustering_method_rows = "ward.D2",
            clustering_method_columns = "ward.D2")%v%
  columnAnnotation(cluster_100 = anno_simple(unfactor(col_anno$cluster_100), pch = unfactor(col_anno$cluster_100),border=TRUE, col = col_cluster_groups,annotation_name_side = "left"))%v%
  columnAnnotation(foo = anno_text(col_anno$names))
...
?Heatmap
  rowAnnotation(meta_PID_groups$)


ha <- columnAnnotation(cluster_100 = anno_simple(unfactor(col_anno$cluster_100), pch = unfactor(col_anno$cluster_100),border=TRUE))
ha %v%  NULL            
                 
                 
                 col=col_clusters$cluster_100))
...
##Perform kmeans clustering - read into that
set.seed(22)
out_k <- kmeans(PID_clr,centers = 16,iter.max = 1000,nstart = 10000)
row_anno$PID_groups <- out_k$cluster
...
Heatmap(PID_clr, name = "CLR", 
        col = col_main,
        split = row_anno$PID_groups,
        show_column_names = T, show_row_names =  F, 
        row_title_side = c("right"),
        border=TRUE,
        clustering_method_rows = "ward.D2",
