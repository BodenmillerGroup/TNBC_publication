...
#Observation: Mean "raw" exprs counts are most interpretable and logical - use for downstream analysis
```
...
#6.1 Heatmap not scaled exprs scores
...
#6.2 Cluster annotation heatmap
...
```{r pgroups, eval = F}
#kick out unclear clustering with little information ("7","10") or little cells (n<2000 cells, "22","24")
PID_groups <- PID_cluster %>% filter(!cluster_100 %in% c("7","10","22","24")) %>% pivot_wider(id_cols = "PID",names_from = "cluster_100",values_from = "fra") %>% column_to_rownames("PID")
...
col_anno <- anno %>% filter(!cluster_100 %in% c("7","10","22","24"))
...
#kmeans clustering
set.seed(22)
out_k <- kmeans(PID_groups,16,nstart = 10000,iter.max = 10000)
...
ggplot(shannon_plot,aes(x=reorder(PID_group,shannon_equi),y=shannon_div,size=number,color=PID_group))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = PID_group), size = 3, position=position_jitter(w=0.1,h=0.1))+
  theme_classic()+
  xlab("PID_group")+
  ylab("Shannon Equitability Index")+
  ggtitle("Epithelial phenotypes", subtitle = paste("p(Kruskal-Wallis) = ",kw_shannon$p))
...
#Plot shannon equitability for PID groups and compare
ggplot(shannon_plot_1,aes(x=reorder(PID_group_clr,shannon_equi),y=shannon_div,size=number,color=PID_group_clr))+
  geom_boxplot(outlier.colour = NA)+
  geom_jitter(aes(color = PID_group_clr), size = 3, position=position_jitter(w=0.1,h=0.1))+
  theme_classic()+
  xlab("PID_group")+
  ylab("Shannon Equitability Index")+
  ggtitle("Epithelial phenotypes", subtitle = paste("p(Kruskal-Wallis) = ",kw_shannon_1$p))
