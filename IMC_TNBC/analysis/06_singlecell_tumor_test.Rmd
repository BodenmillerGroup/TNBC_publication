...
##Overlap of cells in both approaches in Venn diagram
brewer.pal(3,"Paired")

venn.diagram(
  x = list(colnames(sce_1),colnames(sce_2)),
  category.names = c("GMM_panCK","TumorCluster"),
  filename = "/mnt/rcc_volume/TNBC/data_analysis/TNBC_TumorClus_twoapproaches_venn.png",
  output = FALSE,
  imagetype="png" ,
  height = 800, 
  width = 800, 
  ...
  lwd = 1,
  col=c("#1F78B4","#A6CEE3"),
  fill = c(alpha("#A6CEE3",0.3), alpha("#1F78B4",0.3)),
  cex = 0.5,
  fontfamily = "sans",
  cat.cex = 0.3,
...
#1: for sce_1
set.seed(22)
snn.sce_1 <- buildSNNGraph(sce_1[rowData(sce_1)$cluster_channel==TRUE,], k=50, 
                         d = NA, 
                         type = "jaccard",
                         assay.type = "exprs",
                         BNPARAM=AnnoyParam(),
...
#2: for sce_2
set.seed(22)
snn.sce_2 <- buildSNNGraph(sce_2[rowData(sce_2)$cluster_channel==TRUE,], k=50, 
                         d = NA, 
                         type = "jaccard",
                         assay.type = "exprs",
                         BNPARAM=AnnoyParam(),
...
numbering <- data.frame(old = sort(unique(sce_2$cluster)),new = sort(1:33)) #numbering was not sequential - changed!
sce_2$cluster <- numbering$new[match(sce_2$cluster,numbering$old)]

#Save SCEs after clustering
saveRDS(sce_1, "/mnt/rcc_volume/TNBC/data_analysis/sce_TNBC_249_Tumor_GMM_ClusterChannelClus.rds")
saveRDS(sce_2, "/mnt/rcc_volume/TNBC/data_analysis/sce_TNBC_249_Tumor_CC_ClusterChannelClus.rds")
```
...
cell_state_mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster,
                                 statistics = "mean",
                                 use.assay.type = "min_max",
...
tumor_mean_sce <- aggregateAcrossCells(sce, ids = sce$cluster,
                                 statistics = "mean",
                                 use.assay.type = "min_max",
                                 subset.row = rowData(sce)$tumor_channel)
...
# Stability measures
#testCL<- clValid(assay(cluster_mean_sce, "min_max"),nClust = 4:14,clMethods = c("agnes","diana","kmeans","pam"),validation="internal",metric="euclidean", method = "ward")
...
h <- Heatmap(t(assay(cluster_mean_sce, "min_max")),
  ...
  Heatmap(t(assay(tumor_mean_sce, "min_max")),
        col = col_main,
        name= "min-max",
        km = 1,
        show_row_names = T, 
        ...
        clustering_method_rows = "ward.D2",
        clustering_method_columns = "ward.D2")+
  Heatmap(t(assay(cell_state_mean_sce, "min_max")),
        col = col_main,
        name= "min-max",
        km = 1,
        show_row_names = T, 
