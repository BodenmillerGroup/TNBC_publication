...
pretreatment_mc <- colData(sce) %>% as.data.frame() %>% select(PID, spatial_CD8_pheno,pretreatment) %>% unique() %>% select(-PID) %>% group_by(spatial_CD8_pheno) %>% table() %>% as.data.frame
pretreatment_mc <- pretreatment_mc %>% group_by(spatial_CD8_pheno) %>% mutate(fra = Freq/sum(Freq)) 

cur_test <- pretreatment_mc %>% pivot_wider(names_from = spatial_CD8_pheno, id_cols = pretreatment, values_from = Freq) %>% column_to_rownames("pretreatment")
...
ggplot(pretreatment_mc, aes(x = spatial_CD8_pheno, y = fra, fill = pretreatment))+
  ...
  ggtitle(paste0("p (ChiSqu) = ", round(cur_test_out$p.value,2)))
```
...
cur_mean_out_tumor <- aggregateAcrossCells(sce_tumor, ids = DataFrame(sce_tumor$metacluster_combined,sce_tumor$PID),
                     statistics = "mean",
                     use.assay.type = "exprs",
                     subset.row = rowData(sce_tumor)$clean_target %in% c("panCK", "Ecad", "DNA_1", "DNA_2"))

out_tumor <- as.data.frame(t(assay(cur_mean_out_tumor))) 
out_tumor$dna_sum <- rowSums(out_tumor %>% select(DNA_1, DNA_2))
out_tumor$epi_sum <- rowSums(out_tumor %>% select(panCK, Ecad))

out_tumor$metacluster_combined <- cur_mean_out_tumor$sce_tumor.metacluster_combined
out_tumor$PID <- cur_mean_out_tumor$PID
out_tumor$cat <- "all"

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TNBC_MC1_pancK_DNA_expr_comp.pdf",width = 9,height = 4)
ggplot(out_tumor, aes(x = metacluster_combined, y = panCK))+
  geom_jitter(aes(color = PID), size = 3, position=position_jitter(w=0.1,h=0.1))+
  geom_boxplot(outlier.color = NA)+
  guides(color = "none")+
  scale_color_manual(values = metadata(sce)$colors$PID)+
  geom_hline(yintercept = median(out_tumor$panCK), linetype = "dashed", color = "darkgrey")+
  theme_classic()+
  ylab("panCK tumor expression")+
  theme(axis.text.x = element_text(angle = 90), axis.title.x = element_blank())+
  stat_compare_means(ref.group = "tumor_1", label = "p.signif", method = "wilcox.test")+

ggplot(out_tumor, aes(x = metacluster_combined, y = dna_sum))+
  geom_jitter(aes(color = PID), size = 3, position=position_jitter(w=0.1,h=0.1))+
  geom_boxplot(outlier.color = NA)+
  guides(color = "none")+
  scale_color_manual(values = metadata(sce)$colors$PID)+
  geom_hline(yintercept = median(out_tumor$dna_sum), linetype = "dashed", color = "darkgrey")+
  stat_compare_means(ref.group = "tumor_1", label = "p.signif", method = "t.test")+
  ylab("DNA tumor expression")+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90), axis.title.x = element_blank())
dev.off()
```
...
```{r}
## Compare signal on non-tumor cells relative to MC1 tumor appearance 

# Subset to non-tumor cells and focus on mean CD45 and SMA signal across immune cells per PID
sce_non_tumor <- sce[,sce$analysis_cat == "non_tumor"]
#sce_non_tumor <- sce_non_tumor[,!sce_non_tumor$metacluster_combined %in% c("Endothelial", "Immune_low", "Stromal")]

cur_mean_out_non_tumor <- aggregateAcrossCells(sce_non_tumor, ids = sce_non_tumor$PID,
                     statistics = "mean",
                     use.assay.type = "exprs",
                     subset.row = rowData(sce_non_tumor)$clean_target %in% c("CD45", "SMA"))

out_TME <- as.data.frame(t(assay(cur_mean_out_non_tumor))) 
out_TME <- out_TME %>% rownames_to_column("PID")
out_TME$TME_com <- rowSums(out_TME %>% select(SMA, CD45))

# Add number of MC1 cells per PID
n_tumor_1 <- PID_metacluster %>% filter(metacluster_combined == "tumor_1")
out_TME$n_MC1 <- n_tumor_1$Freq[match(n_tumor_1$PID, rownames(cur_df))]

# Plot 
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TNBC_MC1_CD45_SMA_immune_comp.pdf",width = 6,height = 4)
ggplot(out_TME, aes(x = n_MC1, y = SMA))+
  geom_point()+
  geom_smooth(method='lm', formula= y~x, se = TRUE, color = "darkgrey")+
  stat_cor(method = "spearman")+
  theme_classic()+
  guides(color = "none")+
  ylab("SMA TME expression")+
  xlab("MC1 cells / Patient")+
  scale_color_manual(values = metadata(sce)$colors$PID)+

ggplot(out_TME, aes(x = n_MC1, y = CD45))+
  geom_point()+
  geom_smooth(method='lm', formula= y~x, se = TRUE, color = "darkgrey")+
  theme_classic()+
  ylab("CD45 TME expression")+
  xlab("MC1 cells / Patient")+
  stat_cor(method = "spearman")
dev.off()
```
...
# One-hot encoding of clinical variables
one_hot <- as.data.frame(model.matrix(~ . - 1, data = Cox_df %>% select(grade,pT_simple, pN_simple)))
Cox_df <- cbind(Cox_df %>% select(-grade,-pT_simple, -pN_simple),one_hot)
...
Here we just text whether MC8 has significant interaction terms with non-epithelial cells. 

```{r}
PID_metacluster_combined <- colData(sce) %>% as.data.frame() %>% select(metacluster_combined,PID) %>% filter(metacluster_combined %in% c("B_cell", 
"CD4_T_cell", "CD8_T_cell", "Endothelial", "HLA_DR", "Immune_low", "Monocyte", "Neutrophil", "Stromal", "tumor_8"))

PID_metacluster_combined$metacluster_combined <- unfactor(PID_metacluster_combined$metacluster_combined)

PID_metacluster_combined <- PID_metacluster_combined %>% group_by(PID) %>% table() %>% as.data.frame
...
Cox_df_mean <- foreach(i = Cox_df %>% colnames,.combine = "cbind")%do%{
Cox_df %>% select(i) %>% mutate(mean = ifelse(Cox_df[,i] > mean(Cox_df[,i]),"B","A")) %>% pull(mean)
}

colnames(Cox_df_mean) <- Cox_df %>% colnames
rownames(Cox_df_mean) <- Cox_df %>% rownames
Cox_df_mean <- as.data.frame(Cox_df_mean)
identical(rownames(Cox_df), rownames(Cox_df_mean))

#add covariates for correction
Cox_df_mean$status_DFS <- sce$status_DFS[match(rownames(Cox_df_mean),sce$PID)]
Cox_df_mean$DFS_months <- sce$DFS_months[match(rownames(Cox_df_mean),sce$PID)]

#Cox_df_mean$age <- sce$age[match(rownames(Cox_df_mean),sce$PID)]
#Cox_df_mean$grade <- factor(sce$grade[match(rownames(Cox_df_mean),sce$PID)], levels = c("3","2","1"))
#Cox_df_mean$pT_simple <- as.factor(sce$pT_simple[match(rownames(Cox_df_mean),sce$PID)])
#Cox_df_mean$pN_simple <- as.factor(sce$pN_simple[match(rownames(Cox_df_mean),sce$PID)])
#Cox_df_mean$pM_simple <- as.factor(sce$pM_simple[match(rownames(Cox_df_mean),sce$PID)]) 

#Based on proportions
covariates <- Cox_df_mean %>% select(-status_DFS,-DFS_months, -Immune_low) %>% colnames
#covariates <- Cox_df_mean %>% select(-status_DFS,-DFS_months, -age, -grade, -pT_simple, -pN_simple, -pM_simple, -Immune_low) %>% colnames

out_interaction <- list()
for(i in c("B_cell", "CD4_T_cell", "CD8_T_cell", "Endothelial", "HLA_DR", "Immune_low", "Monocyte", "Neutrophil", "Stromal")){
  out_interaction[i] <- paste0("tumor_8:", i)
}

out_interaction <- unlist(out_interaction, use.names = FALSE)

#multi_formula <- as.formula(paste("Surv(DFS_months, status_DFS) ~ age + grade + pT_simple + pN_simple + pM_simple +", paste(covariates, collapse = " + ")," + Immune_low +" ,paste(out_interaction, collapse = " + ")))

multi_formula <- as.formula(paste("Surv(DFS_months, status_DFS) ~ ", paste(covariates, collapse = " + ")," + Immune_low +" ,paste(out_interaction, collapse = " + ")))
#multi_formula <- as.formula(paste("Surv(DFS_months, status_DFS) ~ " ,paste(out_interaction, collapse = " + ")))
...
coord_cartesian(ylim=c(0.001,10000))+
...
write.csv(plot_df_prop, "/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/supp_tables/Cox_Multi_MC8patch.csv")
...
out_df <- colData(sce) %>% as.data.frame() %>% select(PID, DFS_months, status_DFS) %>% unique()

out_df$spatial_CD8_pheno <- out$spatial_CD8_phenotype[match(out_df$PID, out$PID)]
...
#Plot survival curves
fit <- survdiff(Surv(DFS_months, status_DFS) ~ spatial_CD8_pheno, data = out_df)
fit_1 <- survdiff(Surv(DFS_months, status_DFS) ~ spatial_CD8_pheno_merg, data = out_df)
...
cell_fun = function(j, i, x, y, width, height, fill) {
  if(as.matrix(heatmap_out)[i,j] < 0.05){
      grid.points(x, y, pch = 8, size = unit(4, "mm"))}
  if(as.matrix(heatmap_out)[i,j] < 0.1){
      grid.points(x, y, pch = 4, size = unit(4, "mm"))
        }
}

cell_fun_merg = function(j, i, x, y, width, height, fill) {
  if(as.matrix(heatmap_out_merg)[i,j] < 0.02){
      grid.points(x, y, pch = 8, size = unit(4, "mm"))}
  if(as.matrix(heatmap_out_merg)[i,j] < 0.05){
      grid.points(x, y, pch = 4, size = unit(4, "mm"))
        }
}

heatmap_out <- list_out_df %>% pivot_wider(id_cols = infiltrating, names_from = ratio_inf_ex, values_from = p) %>% column_to_rownames("infiltrating")
Heatmap(heatmap_out, cell_fun = cell_fun, col = rev(viridis(100)), cluster_columns = FALSE, cluster_rows = FALSE, border = TRUE)

heatmap_out_merg <- list_out_df %>% pivot_wider(id_cols = infiltrating, names_from = ratio_inf_ex, values_from = p_merg) %>% column_to_rownames("infiltrating")
...
```{r}
## Classify Patients based on spatial CD8 T cell scores

## Use three ratios for comparison (alongside ratio 10 and inf 20 = final choice)

# 1. Ratio 10 
out <- cur_dat %>% group_by(PID, CD8Tcell_type) %>% dplyr::count() %>% 
  pivot_wider(id_cols = PID, values_from = n, names_from = CD8Tcell_type) %>% 
  mutate(ratio_inf_ex = bystanding / infiltrating, number_inf_ex = bystanding + infiltrating) %>% 
  mutate(spatial_CD8_phenotype = ifelse(ratio_inf_ex >= 2 | infiltrating <= 20, "excluded", "inflamed")) %>% 
  mutate(spatial_CD8_phenotype = ifelse(number_inf_ex <= 10 | is.na(number_inf_ex), "cold", spatial_CD8_phenotype))
...
# Add to sce
sce$spatial_CD8_pheno_2 <- out$spatial_CD8_phenotype[match(sce$PID, out$PID)]
sce[,is.na(sce$spatial_CD8_pheno_2)]$spatial_CD8_pheno_2 <- "cold"

sce$spatial_CD8_pheno_2_merg <- ifelse(sce$spatial_CD8_pheno_2 == "inflamed", "inflamed", "cold_excluded")
...
out <- cur_dat %>% group_by(PID, CD8Tcell_type) %>% dplyr::count() %>% 
  pivot_wider(id_cols = PID, values_from = n, names_from = CD8Tcell_type) %>% 
  mutate(ratio_inf_ex = bystanding / infiltrating, number_inf_ex = bystanding + infiltrating) %>% 
  mutate(spatial_CD8_phenotype = ifelse(ratio_inf_ex >= 6 | infiltrating <= 16, "excluded", "inflamed")) %>% 
  mutate(spatial_CD8_phenotype = ifelse(number_inf_ex <= 10 | is.na(number_inf_ex), "cold", spatial_CD8_phenotype))
...
# Add to sce
sce$spatial_CD8_pheno_16 <- out$spatial_CD8_phenotype[match(sce$PID, out$PID)]
sce[,is.na(sce$spatial_CD8_pheno_16)]$spatial_CD8_pheno_16 <- "cold"

sce$spatial_CD8_pheno_16_merg <- ifelse(sce$spatial_CD8_pheno_16 == "inflamed", "inflamed", "cold_excluded")
```


```{r}
## Spatial_CD8_pheno
cur_surv <- colData(sce) %>% as.data.frame() %>% select(PID, spatial_CD8_pheno_16, spatial_CD8_pheno_16_merg, spatial_CD8_pheno_2,spatial_CD8_pheno_2_merg, OS_months, DFS_months, status_DFS, status_OS) %>% unique()

#Plot survival curves
fit <- survfit(Surv(DFS_months, status_DFS) ~ spatial_CD8_pheno_16, data = cur_surv)
print(fit)

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Spatial_CD8_6_16_DFS_Kaplan.pdf",width = 5,height = 7, onefile = FALSE)
ggsurvplot(fit,
          pval = TRUE,
          risk.table = TRUE, # Add risk table
          risk.table.col = "strata", # Change risk table color by groups
...
#Plot survival curves
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Spatial_CD8_merg_6_16_DFS_Kaplan.pdf",width = 5,height = 7, onefile = FALSE)
fit <- survfit(Surv(DFS_months, status_DFS) ~ spatial_CD8_pheno_16_merg, data = cur_surv)
print(fit)
...
#Plot survival curves
fit <- survfit(Surv(DFS_months, status_DFS) ~ spatial_CD8_pheno_2, data = cur_surv)
print(fit)

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Spatial_CD8_2_20_DFS_Kaplan.pdf",width = 5,height = 7, onefile = FALSE)
ggsurvplot(fit,
          pval = TRUE,
          risk.table = TRUE, # Add risk table
          risk.table.col = "strata", # Change risk table color by groups
...
#Plot survival curves
fit <- survfit(Surv(DFS_months, status_DFS) ~ spatial_CD8_pheno_2_merg, data = cur_surv)
print(fit)

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Spatial_CD8_merg_2_20_DFS_Kaplan.pdf",width = 5,height = 7, onefile = FALSE)
ggsurvplot(fit,
          pval = TRUE,
          risk.table = TRUE, # Add risk table
          risk.table.col = "strata", # Change risk table color by groups
...
sc#### Number of T cells comparison 

```{r}
# Compare number of CD8+ T cells between spatial CD8 T cell groups

out <- colData(sce) %>% as.data.frame %>% select(PID, spatial_CD8_pheno, metacluster_tumor_combined) %>% filter(metacluster_tumor_combined == "CD8_T_cell") %>% group_by(PID, spatial_CD8_pheno) %>% dplyr::count() 
...
#Plot survival curves
fit <- survfit(Surv(OS_months, status_OS) ~ CD8_number_category, data = cur_surv)
print(fit)
...
#Plot survival curves
fit <- survfit(Surv(OS_months, status_OS) ~ CD8_number_category_merg, data = cur_surv)
print(fit)
...
app <- cytoviewer(image = cur_images, mask = cur_mask, object = cur_sce, cell_id = "ObjectNumber",img_id = "sample_id")
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_PID.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "PID", point_size = 1)+#shape_by = "analysis_cat")+
  scale_color_manual(values = metadata(sce)$colors$PID)+
  guides(color = "none")
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_TMANr.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "TMANr", point_size = 1)+#shape_by = "analysis_cat")+
  scale_color_manual(values = metadata(sce)$colors$TMANr)+
  guides(color = "none")
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_NAT.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "pretreatment", point_size = 1)+#shape_by = "analysis_cat")+
  scale_color_manual(values = metadata(sce)$colors$pretreatment)+
  guides(color = "none")
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_CC.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "cluster_category", point_size = 1)+#shape_by = "analysis_cat")+
  scale_color_manual(values = metadata(sce)$colors$cluster_category)+
  guides(color = "none")
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_analysiscat.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "analysis_cat", point_size = 1)+#shape_by = "analysis_cat")+
  scale_color_manual(values = metadata(sce)$colors$analysis_cat)+
  guides(color = "none")
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_TMEmetacluster.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "metacluster_tumor_combined", point_size = 1)+#shape_by = "analysis_cat")+
  scale_color_manual(values = color_TME, limits = force)+
  guides(color = "none")
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_TUMORmetacluster.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "metacluster_TME_combined", point_size = 1)+#shape_by = "analysis_cat")+
  scale_color_manual(values = color_TME, limits = force)+
  guides(color = "none")
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_panCK.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "panCK", by_exprs_values = "exprs", point_size = 1)
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/TSNE_Ecad.pdf",width = 3.5,height = 3.5, onefile = FALSE)
plotReducedDim(sce_sub, dimred = "TSNE", colour_by = "Ecad", by_exprs_values = "exprs", point_size = 1)
dev.off()
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Spatial_CD8_DFS_Cox_Multi.pdf",width = 7,height = 5, onefile = FALSE)
ggplot(plot_df_prop, aes(x = reorder(group,desc(HR)), y = HR, color = p_value < 0.05))+
geom_point(size = 4)+
geom_errorbar(aes(ymax = CI_high, ymin = CI_low))+
geom_hline(yintercept = 1,color = "black",linetype = "dashed")+
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Spatial_CD8_NAT_phenotype.pdf",width = 5,height = 4, onefile = FALSE)
ggplot(pretreatment_mc, aes(x = spatial_CD8_pheno, y = fra, fill = pretreatment))+
  geom_tile(position = "stack",color = "black")+#+alpha = 0.5)+
  theme_classic()+
  ylab("Fraction of cells per group")+
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Spatial_CD8_Bpatchsize.pdf",width = 4,height = 5, onefile = FALSE)
ggplot(plot) + 
  geom_col(aes(y=spatial_CD8_pheno, x=fraction, fill=B_cell_score), color = "black") +
  theme_classic() + 
  xlab("Fraction of PID") +
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Tumor_exprs_SpatialCD8.pdf",width = 5,height = 5, onefile = FALSE)
draw(h,annotation_legend_list = list(lgd_cat))
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Cox_Tumor_TME_Lasso_Uni.pdf",width = 6,height = 3, onefile = FALSE)
lasso_plot
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Cox_Tumor_TME_Lasso_Multi.pdf",width = 6,height = 3, onefile = FALSE)
lasso_plot
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Numbergroup_SpatialGroup_Bar.pdf",width = 5,height = 4, onefile = FALSE)
ggplot(out_viz, aes(x = CD8_category, y = fra, fill = spatial_CD8_pheno))+
  geom_bar(stat = "identity", color = "black")+
  theme_classic()+
  scale_fill_manual(values = metadata(sce)$colors$spatial_CD8_pheno)
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Numbergroup_Kaplan.pdf",width = 5,height = 7, onefile = FALSE)
ggsurvplot(fit,
          pval = TRUE,
          risk.table = TRUE, # Add risk table
          risk.table.col = "strata", # Change risk table color by groups
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Numbergroup_MultiCox.pdf",width = 7,height = 5, onefile = FALSE)
ggplot(plot_df_prop, aes(x = reorder(group,dplyr::desc(HR)), y = HR, color = p_value < 0.05))+
geom_point(size = 4)+
geom_errorbar(aes(ymax = CI_high, ymin = CI_low))+
coord_cartesian(ylim=c(0.1,100))+
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/SpatialGroup_DiffAbun_Tumor_TME.pdf",width = 7,height = 4, onefile = FALSE)
ggplot(DA, aes(x = cluster, y = category, col = logFC, size = -log10(FDR))) +
  geom_point() +
  theme_classic() +
  scale_color_gradient2(high = "#E41A1C", mid = "white", low = "#377EB8") +
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/SpatialGroup_DiffInteraction_Tumor_TME.pdf",width = 7,height = 4, onefile = FALSE)
Heatmap(pVal,
          col = col,
          name = "log10(p)",
          row_split = anno$type,
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/07_combined/Cox_InteractionTerms_MC8_Immune.pdf",width = 5,height = 3, onefile = FALSE)
hr_prop
dev.off()
