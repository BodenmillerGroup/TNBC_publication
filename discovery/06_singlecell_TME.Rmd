...
sce <- readRDS("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/06_sce_TNBC_TME_new.rds")
...
pretreatment_mc <- colData(sce) %>% as.data.frame() %>% select(immune_metacluster,pretreatment) %>% group_by(immune_metacluster) %>% table() %>% as.data.frame
pretreatment_mc <- pretreatment_mc %>% group_by(immune_metacluster) %>% mutate(fra = Freq/sum(Freq)) 

ggplot(pretreatment_mc, aes(x = immune_metacluster, y = fra, fill = pretreatment))+
...
plotPixels(cur_images,
           ...
           save_plot = list(filename = paste0("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/TME_clusters_viz/TME_viz_cluster_",i,".png"), scale = 2)
           )
...
fit <- survfit(Surv(OS_months, status_OS) ~ CD8_T_cell, data = Cox_df_mean)
print(fit)
...
PID_metacluster <- colData(sce) %>% as.data.frame() %>% filter(pM_simple != 1) %>% select(immune_metacluster,PID) %>% group_by(PID) %>% table() %>% as.data.frame
PID_metacluster <- PID_metacluster %>% group_by(PID) %>% mutate(fra = Freq/sum(Freq))
...
#Based on proportions
covariates <- Cox_df %>% select(-status_DFS,-DFS_months, -age, -grade, -pT_simple, -pN_simple, -Immune_low) %>% colnames

multi_formula <- as.formula(paste("Surv(DFS_months, status_DFS) ~ age + grade + pT_simple + pN_simple +", paste(covariates, collapse = " + "),"+ Immune_low"))
...
geom_point(size = 4)+
geom_errorbar(aes(ymax = CI_high, ymin = CI_low))+
coord_cartesian(ylim=c(0.0001,100000))+
geom_hline(yintercept = 1,color = "black",linetype = "dashed")+
scale_color_brewer(palette = "Set1")+
xlab("Immune metaclusters")+
ylab("HR for con_prop")+
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_Heatmap_ClusterTesting.pdf",height = 4, width = 9)
p1+p2+p3 
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_Silhouette_Cluster130.pdf",width = 12,height = 10)
p4
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_HeatmapCluster.pdf",width = 10,height = 6)
draw(h,annotation_legend_list = list(lgd_astir, lgd_meta))
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_HeatmapCluster_Scales.pdf",width = 13,height = 8)
h_exprs + h_min_max
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_ClusterAnno.pdf",width = 5,height = 8)
h_cluster_anno
dev.off()
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_ExpressionCor.pdf",width = 10,height = 8)
corrplot(cor(cor_sce,method = "spearman"), method="color", col=brewer.pal(n=8, name="RdBu"),  
         type="upper", order="hclust",hclust.method = "ward.D2", 
         addCoef.col = "black", addCoefasPercent = F,# Add coefficient of correlation
         tl.col="black", tl.srt=40, #Text label color and rotation
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_TSNECluster.pdf",width = 8,height = 6)
TSNE_clus
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_TSNEExpression.pdf",width = 12,height = 6)
exp_plots_TSNE
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_UMAPExpression.pdf",width = 12,height = 6)
exp_plots_UMAP
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_ClusterPIDSID.pdf",width = 12,height = 6)
p_clusterPID
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_PIDCluster_ClusterCor.pdf",width = 14,height = 6)
(d_PID + SID_immune_metacluster_cor_plot)/
(d_PID_dens + SID_metacluster_dens_cor_plot)
dev.off()
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_CC_Cluster_HR_DFS.pdf",width = 8,height = 10)
hr_prop/
hr_cat/
hr_cat_GMM
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_MC_HR_Multi_DFS.pdf",width = 6,height = 3, onefile = FALSE)
hr_prop
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_FractionHeatmap.pdf", width = 8, height = 18, onefile = FALSE)
fraction_anno + NULL
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_Metacluster_Fra_Cor.pdf", width = 6)
SID_immune_metacluster_cor_plot
dev.off()


pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_clrDR_MetaclusterPID.pdf", height = 5, width = 6)
clrDR(sce_cat, dr = "PCA", 
      by = "cluster_id", k = "immune_metacluster", 
      label_by = "cluster_id", arrow_col = "PID", 
      point_pal = metadata(sce_cat)$colors$immune_metacluster, 
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_clrDR_PIDMetacluster.pdf", height = 5, width = 6)
clrDR(sce_cat, dr = "PCA", 
      by = "sample_id", point_col = "sample_id",
      k = "immune_metacluster", point_pal = metadata(sce_cat)$colors$PID)+
  scale_color_manual(values = metadata(sce_cat)$colors$immune_metacluster)+
...
pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_Cox_HR_DFS_Meta_Multi.pdf", height = 4, width = 8)
hr_prop
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_Cox_HR_DFS_Meta_Mean.pdf", height = 3, width = 6)
hr_mean
dev.off()

pdf("/mnt/central_nas/tnbc_volume/TNBC/data_analysis/01_figures/06_TME/TNBC_TME_MC_HR_Multi_DFS_M1out.pdf", height = 3, width = 6)
hr_prop
dev.off()
